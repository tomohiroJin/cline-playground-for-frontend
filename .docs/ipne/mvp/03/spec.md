...
- 罠は **「利用・判断対象」** として設計
- テレポート罠は「窮地からの脱出」または「運任せのショートカット」を想定
...
| 通過 | 不可（破壊後は可能） |
| HP | 3 |
| 攻撃 | 有効（プレイヤーの攻撃力分のダメージ） |
| 配置数 | 20個（通路間の壁を置換） |
| 見た目 | ひび割れ模様（戦士は攻撃して初めてわかる） |
| 攻撃条件 | 敵がいない場合のみ壁を攻撃対象とする |
...
|------|-----|
| 合計数 | 25体 + ボス1体 |
| 構成 | 巡回型10体、突進型6体、遠距離型5体、標本型4体 |
| 配置 | 部屋ごとに1〜4体（部屋サイズに応じて） |

### 8.4 敵パラメータ一覧（バランス調整後）
