...
- パラメータ微調整（実施済み）
  - 敵HP・速度調整（パトロール:HP4, チャージ:HP3, レンジド:HP3/速度1.5, スペシメン:速度4, ボス:速度1.5）
  - レンジド敵スポーン追加（5体）
  - 罠の再使用可能化（全罠タイプ）
  - 特殊アイテムスポーン追加（全回復1, レベルアップ2, マップ公開1）
  - 破壊可能壁への攻撃機能追加
- プレイ時間が5〜10分に収まるか確認
- 繰り返しプレイの意味があるか確認
...
- [x] レベルアップが行動選択に影響する（5択の選択肢）
- [x] 職業によって迷路の見え方・攻略法・初期能力が変わる
- [x] 罠が「避ける対象」から「判断対象」になる（再使用可能化で戦術的利用が可能に）
- [x] ボスを倒してクリアできる
- [x] プレイ時間が5〜10分に収まる（要検証）
- [x] 繰り返し遊ぶ意味がある（異なる職業・成長方針）（要検証）
