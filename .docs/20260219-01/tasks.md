...
- **受入条件**:
  - [ ] 9 枚の WebP 画像が生成されている
  - [ ] 各画像が 960×540 px である
  - [ ] ファイルサイズが 50〜150 KB の範囲内
  - [ ] 画像内にテキストが含まれていない
...
- **受入条件**:
  - [ ] Canvas API によるプレースホルダー生成コードが削除されている
  - [ ] 9 枚の画像が import されている
  - [ ] `getStoryImage()` が import 済みの画像パスを返す
  - [ ] `hasStoryImage()` が正しく動作する
...
- **受入条件**:
  - [ ] 全 8 シーン（プロローグ 3 + ステージ 5）で正しい画像が表示される
  - [ ] 画像が正しいタイミング・シーンで表示される
  - [ ] フェードインアニメーションが動作する
  - [ ] モバイル表示で問題なし
...
- **受入条件**:
  - [ ] `storyImages.ts` 以外のファイルに変更が不要であることを確認
