...
- **受入条件**:
  - [ ] `PROLOGUE_STORY.slides` に3シーン分のデータがある
  - [ ] 各シーンに `imageKey` が設定されている
  - [ ] `PROLOGUE_STORY.lines` が既存のまま維持されている
...
- **作業内容**:
  ...
  - spec.md のテキスト全文を反映
- **受入条件**:
  - [ ] 全5ステージのストーリーが拡充されている
  - [ ] 各ストーリーの `imageKey` が設定されている
  - [ ] `getStageStory()` が正しいデータを返す
...
- **作業内容**:
  ...
  - spec.md のテキスト全文を反映
- **受入条件**:
  - [ ] 全5ランクの `paragraphs` が設定されている
  - [ ] 既存の `text` フィールドが維持されている
  - [ ] `getEpilogueText()`, `getEndingEpilogue()` が正しいデータを返す
...
- **作業内容**:
  ...
  - spec.md のテキスト全文を反映
- **受入条件**:
  - [ ] `GAME_OVER_TEXT.paragraphs` が設定されている
  - [ ] 既存の `text` フィールドが維持されている
...
- **受入条件**:
  ...
  - [ ] 「次へ」ボタンで手動進行できる
  - [ ] 「スキップ」ボタンで即座に完了する
  - [ ] `slides` が未設定の `StoryScene` でも動作する
...
- **受入条件**:
  - [ ] `imageKey` が設定されたストーリーで画像が表示される
  - [ ] 画像がフェードインする
  - [ ] `imageKey` が未設定でもエラーなく動作する
  - [ ] テキストとの上下レイアウトが崩れない
...
- **受入条件**:
  - [ ] 各要素が段階的にフェードインする
  - [ ] `paragraphs` が段落ごとにフェードインする
  - [ ] スキップで全要素が即時表示される
  - [ ] `paragraphs` がない場合でも正常に動作する
...
- **受入条件**:
  - [ ] 4方向それぞれに2フレームの攻撃スプライトがある
  - [ ] 剣を振る動作がドット絵で表現されている
  - [ ] `SpriteSheetDefinition` 形式でエクスポートされている
...
- **受入条件**:
  - [ ] 各方向でのけぞりポーズが表現されている
  - [ ] `SpriteDefinition` 形式でエクスポートされている
...
- **受入条件**:
  - [ ] 既存の idle フレームと視覚的に連続的
  - [ ] 変化が 1-2px の微小な動き
  - [ ] `SpriteSheetDefinition` 形式でエクスポート（frameDuration: 800ms）
...
- **作業内容**:
  ...
  - 赤系/オレンジ系の独自パレットを定義
- **受入条件**:
  - [ ] 2種類のエフェクトスプライトシートが定義されている
  - [ ] `SpriteSheetDefinition` 形式でエクスポートされている
  - [ ] 視覚的にプレイヤー攻撃エフェクトと区別可能
...
- **作業内容**:
  ...
  - アニメーションタイミング管理
- **受入条件**:
  - [ ] プレイヤー攻撃時に攻撃アニメーションが再生される
  - [ ] プレイヤー被弾時に被弾フレームが 200ms 表示される
  - [ ] 待機中にブリーズアニメーションが再生される
  - [ ] 敵の状態変化に応じてフレームが切り替わる
...
- **受入条件**:
  - [ ] 3関数が正しくパーティクル配列を生成する
  - [ ] パーティクルの初期位置・速度が仕様通り
  - [ ] 既存の `updateParticles()`, `drawParticles()` で正しく処理される
...
- **受入条件**:
  - [ ] 近接攻撃エフェクトが赤系パーティクルで表示される
  - [ ] 遠距離攻撃エフェクトが軌跡パーティクルで表示される
  - [ ] ボス攻撃エフェクトがパルス波パーティクルで表示される
  - [ ] 既存の `addEffect()` 呼び出しが影響を受けない
...
- **作業内容**:
  ...
  - リニア減衰 200ms
- **受入条件**:
  - [ ] `getShakeOffset()` がシェイク中に `{x, y}` を返す
  - [ ] シェイク終了後に `null` を返す
  - [ ] 振幅がリニアに減衰する
...
- **受入条件**:
  - [ ] HP 25% 以下で画面縁に赤いパルスが表示される
  - [ ] HP 25% 超で非表示
  - [ ] パルスが 1500ms 周期でアニメーションする
  - [ ] 画面中央はクリアに見える（ビネット効果）
...
- **受入条件**:
  - [ ] 各エフェクトが正しいタイミングでトリガーされる
  - [ ] 画面シェイクがゲーム描画に正しく適用される
  - [ ] HUD は画面シェイクの影響を受けない
  - [ ] 低HP警告が描画される
...
- **作業内容**:
  ...
  - 長時間プレイでのメモリリーク検証
- **受入条件**:
  - [ ] エフェクト多重発生時に 30fps 以上を維持
  - [ ] パーティクル上限到達時に古いエフェクトが正しく削除される
  - [ ] 10 分連続プレイでメモリ使用量が安定
