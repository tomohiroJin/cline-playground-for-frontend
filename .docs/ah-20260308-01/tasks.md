...
**受け入れ基準**:
- [x] タイトル画面に「ストーリー」ボタンがある
- [x] 「START」ボタンのテキストが「フリー対戦」に変更されている
- [x] ストーリーボタン押下で `onStoryClick` コールバックが呼ばれる
- [x] 既存のフリー対戦フローが変わらない
**依存**: なし
...
**受け入れ基準**:
- [x] `screen` 型に `'stageSelect' | 'preDialogue' | 'vsScreen' | 'postDialogue'` が追加
- [x] `gameMode` state が `'free' | 'story'` で管理される
- [x] `currentStage` state が現在のステージ情報を保持する
- [x] ストーリーボタン押下で `gameMode='story'`, `screen='stageSelect'` に遷移
**依存**: P3-01
...
**受け入れ基準**:
- [x] 章タイトル「第1章 はじめの挑戦」がヘッダーに表示される
- [x] 3ステージがカード形式で縦並びに表示される
- [x] 各カードに「ステージ番号」「ステージ名」「対戦相手名」「フィールド名」「難易度（★表示）」がある
- [x] クリア済みカードに ✅ チェックマークがある
- [x] 未解放カードは暗転 + 🔒 表示、タップ不可
- [x] 解放済みカードをタップで `onSelectStage(stage)` が呼ばれる
- [x] 「戻る」ボタンでタイトル画面に遷移
- [x] 「リセット」ボタンで進行データをクリア（確認ダイアログ付き）
- [x] キャラアイコン画像が各カードに表示される（画像未配置時はフォールバック表示）
**依存**: P2-07, P2-08, P3-02
**テストファイル**: `components/StageSelectScreen.test.tsx`（14テスト）
...
**受け入れ基準**:
- [x] 画面全体に半透明の暗転オーバーレイ
- [x] 画面下部にテキストボックス（角丸、半透明背景）
- [x] テキストボックス内にキャラアイコン + 名前 + セリフ
- [x] セリフが1文字ずつ表示される（30ms/文字）
- [x] 表示中にタップ → 全文が即座に表示される
- [x] 全文表示後にタップ → 次のセリフへ進む
- [x] 最後のセリフの後にタップ → `onComplete()` 呼出
- [x] 「スキップ」ボタンで即座に `onComplete()`
- [x] キャラアイコン画像が存在しない場合、キャラ名の頭文字がフォールバック表示
**依存**: P2-02, P3-02
**テストファイル**: `components/DialogueOverlay.test.tsx`（9テスト）

### P3-05: VS 画面 ✅
**ファイル**: 新規 `components/VsScreen.tsx`
**内容**: 対決演出画面（2秒自動遷移）
**受け入れ基準**:
- [x] 画面中央に「VS」の大文字（白、ボールド、40px以上）
- [x] 左側にプレイヤーキャラアイコン + 名前、右側に対戦相手キャラアイコン + 名前
- [x] 下部にステージ名とフィールド名
- [x] 300ms フェードインで表示
- [x] 2秒間表示後、300ms フェードアウト
- [x] フェードアウト完了で `onComplete()` 呼出
**依存**: P2-02, P3-02
**テストファイル**: `components/VsScreen.test.tsx`（7テスト）

### P3-06: ストーリーモード遷移統合 ✅
**ファイル**: `AirHockeyGame.tsx`
**内容**: stageSelect → preDialogue → vsScreen → game → postDialogue → result の画面遷移を実装
**受け入れ基準**:
- [x] ステージ選択でステージを選ぶと `preDialogue` 画面に遷移
- [x] ダイアログ完了で `vsScreen` に遷移
- [x] VS 画面完了で `game` 画面に遷移（カウントダウン開始）
- [x] ゲーム中の難易度・フィールド・勝利スコアがステージ定義に従う
- [x] 試合終了で `postDialogue` 画面に遷移（勝敗に応じたダイアログ）
- [x] ダイアログ完了で `result` 画面に遷移
- [x] 勝利時、ストーリー進行が保存される（次ステージ解放）
- [x] リザルト画面「ステージ選択」ボタンで `stageSelect` に遷移
- [x] リザルト画面「リプレイ」で同じステージを再プレイ
- [x] スコアボードの「メニュー」ボタンでタイトル画面に戻れる
**依存**: P3-01 〜 P3-05
...
**受け入れ基準**:
- [x] ストーリーモード時、リザルト画面に「ステージ選択」ボタンが表示される
- [x] フリー対戦時は既存の「BACK TO MENU」のみ
- [x] 「リプレイ」ボタンはストーリーモードでも機能する（同じステージを再プレイ）
- [x] 勝利時に「次のステージへ」ボタンが表示される（最終ステージ以外）
**依存**: P3-06

### P3-08: Phase 3 テスト ✅
**ファイル**: `components/StageSelectScreen.test.tsx`, `components/DialogueOverlay.test.tsx`, `components/VsScreen.test.tsx`, `core/story-mode.test.ts`
**内容**: ストーリーモード全体の統合テスト
**受け入れ基準**:
- [x] ステージ選択画面のコンポーネントテスト（解放状態の表示）
- [x] ダイアログオーバーレイのコンポーネントテスト（文字送り、スキップ）
- [x] VS 画面のコンポーネントテスト（タイマー遷移）
- [x] ストーリー進行保存のテスト（勝利でクリアフラグ保存、次ステージ解放）
- [x] フリー対戦の既存テストが全て通る（全296テスト PASS）
**依存**: P3-01 〜 P3-07
**リファクタリング**: キャラアイコン表示を `CharacterAvatar` コンポーネントに共通化
...
**受け入れ基準**:
- [x] 全テスト PASS（355テスト、既存296 + 新規59）
- [x] 新規テストのカバレッジ 80% 以上（story-balance.ts: 100%, ai.ts: 97.5%, characters.ts: 100%, story.ts: 100%）

**新規テストファイル**:
- `core/story-balance.test.ts`（30テスト）: バランス設定、AI プリセット、ステージ別パラメータ
- `core/ai-configurable.test.ts`（8テスト）: AiBehaviorConfig インターフェース、後方互換性
- `core/integration.test.ts`（21テスト）: クロスモジュール統合、フロー検証、回帰テスト

### P4-02: クロスブラウザ確認 🔧（手動確認待ち）
**内容**: Chrome, Safari, Firefox でのプレイテスト
**受け入れ基準**:
- [ ] 各ブラウザでビジュアルが正常
- [ ] タッチ操作が正常（モバイル Chrome / Safari）
- [ ] キーボード操作が正常
**備考**: P4-02 はブラウザでの手動確認が必要

### P4-03: バランス調整 ✅
**内容**: ストーリーモード各ステージの難易度微調整
**受け入れ基準**:
- [x] ステージ1-1 は初心者が2-3回で勝てる（CPU速度1.2, スキップ率10%, ウォブル40px, 中央寄せ80%）
- [x] ステージ1-2 はアイテム活用で勝てる（アイテム出現4秒間隔, カムバック閾値2）
- [x] ステージ1-3 は苦戦するが練習すれば勝てる（CPU速度5.0, 壁バウンス予測あり, カムバック補正強化）
- [x] カムバック補正が適切に機能する（ステージ1-2/1-3でスコア差2から発動、マレットボーナス15%）

**新規ファイル**:
- `core/story-balance.ts`: ステージ別バランス設定・AI 振る舞いプリセット

**リファクタリング**:
- `core/AI.ts` を `AiBehaviorConfig` ベースに再設計
  - `calculateTargetWithBehavior()` / `updateWithBehavior()` メソッド追加
  - ハードコードされた難易度分岐を設定ベースのロジックに置換
  - 既存 API（`calculateTarget()` / `update()`）は委譲パターンで後方互換を維持

### P4-04: ドキュメント更新 ✅
**内容**: プロジェクトドキュメント整備
**受け入れ基準**:
- [x] `src/features/air-hockey/README.md` の更新（ストーリーモード情報追加）
- [x] `src/features/air-hockey/doc/story-mode.md` 新規作成（バランス設定・キャラクター・画面遷移）
- [x] `src/features/air-hockey/doc/architecture.md` 更新（ファイル構成・テスト数）
- [x] `src/features/air-hockey/doc/features.md` 更新（ストーリーモード・演出セクション追加）
