...
**受け入れ基準**:
- [ ] `lightenColor('#3498db', 40)` が明るい青を返す
- [ ] `darkenColor('#3498db', 40)` が暗い青を返す
- [ ] `#` 付きの6桁HEXを受け取り `rgb()` 文字列を返す
**依存**: なし
...
**受け入れ基準**:
- [ ] 外枠が暗い茶〜黒のグラデーション（幅12px）
- [ ] 内側に 1px の光沢ハイライト線
- [ ] フィールドカラーのネオン枠線が内側に2px幅で描画
- [ ] 6フィールドすべてで正常表示
**依存**: なし
...
**受け入れ基準**:
- [ ] フィールド中央が微かに明るく、端が微かに暗い
- [ ] 障害物の視認性を阻害しない（透明度十分低い）
**依存**: P1-02
...
**受け入れ基準**:
- [ ] 中央ラインが3本線（太い半透明 + 上下の細い線）
- [ ] 中央円内に小さな塗りつぶし円がある
- [ ] 既存の中央円ネオングロー（68-74行目）は維持
**依存**: P1-02
...
**受け入れ基準**:
- [ ] ゴールラインに沿ってドット状の発光がある
- [ ] CPU 側（上）が赤、プレイヤー側（下）がシアン
- [ ] `field.goalSize` に応じたドット数
- [ ] 各フィールドのゴールサイズ（105〜180px）で正常表示
**依存**: P1-02
...
- [ ] マレット下に楕円シャドウがある
- [ ] 外周にエッジリングがある
- [ ] プレイヤー（`#3498db`）とCPU（`#e74c3c`）で色が正しい
- [ ] `sizeScale=1.5`（Big アイテム）で崩れない
- [ ] `hasGlow=true`（Speed エフェクト）のマゼンタグローが描画される
**依存**: P1-01
...
**受け入れ基準**:
- [ ] パックにメタリック感（中心が明るく外周が暗い）がある
- [ ] 速度による色変化（白→黄→赤）が維持されている
- [ ] 高速時のグロー（`shadowBlur`）が維持されている
- [ ] 小さなハイライトスポットがある
- [ ] 透明パック（`visible=false`）は描画されない
**依存**: P1-01
...
**受け入れ基準**:
- [ ] `HitStopState` 型が export されている
- [ ] `active`, `framesRemaining`, `impactX`, `impactY`, `shockwaveRadius`, `shockwaveMaxRadius` フィールドを持つ
**依存**: なし
...
**受け入れ基準**:
- [ ] `hitStopRef` が `useRef<HitStopState>` で初期化される
- [ ] パック速度 > 8 のヒット時に `active=true`, `framesRemaining=3` が設定される
- [ ] `active=true` の間、物理更新（パック移動、AI更新等）がスキップされる
- [ ] 描画は継続される（衝撃波含む）
- [ ] 3フレーム後に自動解除
- [ ] SE（ヒット音）は通常通り発動
**依存**: P1-08
...
**受け入れ基準**:
- [ ] 衝突地点から拡がる半透明の白リング（3フレームで最大80px）
- [ ] 内側にもう1つの薄いオレンジリング
- [ ] 拡がるにつれて透明度が下がる
**依存**: P1-09
...
**受け入れ基準**:
- [ ] `SlowMotionState` 型が export されている
- [ ] `active`, `startTime`, `duration` フィールドを持つ
**依存**: なし
...
**受け入れ基準**:
- [ ] `slowMoRef` が `useRef<SlowMotionState>` で初期化される
- [ ] ゴール判定時に `active=true`, `duration=400` が設定される
- [ ] `active=true` の間、パック/マレットの移動速度が 0.3 倍になる
- [ ] 400ms 後に自動解除、通常速度に復帰
- [ ] スロー中はビネット描画が呼ばれる
- [ ] 自分の得点でも相手の得点でも発動する
**依存**: P1-11
...
**受け入れ基準**:
- [ ] 画面四隅が暗くなる放射グラデーション
- [ ] `intensity` パラメータ（0-1）で濃さを制御可能
- [ ] スローモーション中のみ呼ばれる
**依存**: P1-12

### P1-14: Phase 1 テスト
**ファイル**: `core/phase1.test.ts` 他既存テスト
**内容**: 既存テストの実行確認 + ヒットストップ・スローモーションのユニットテスト追加
**受け入れ基準**:
- [ ] 既存テストが全て通る（`npm test -- air-hockey` でエラーなし）
- [ ] ヒットストップ: 速度 > 8 で発動、3フレームで解除のテスト
- [ ] スローモーション: ゴール時に発動、400ms で解除のテスト
- [ ] `lightenColor` / `darkenColor` のユニットテスト
**依存**: P1-01 〜 P1-13
