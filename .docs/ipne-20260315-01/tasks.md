...
## Phase 7: 統合テスト・シナリオテストの導入 ✅ 完了（2026-03-16）

### P7-1: テストヘルパーの作成 ✅

- [x] **P7-1-1**: `__tests__/helpers/scenarioHelpers.ts` を作成
  - `SeededRandomProvider` — xorshift32 ベースの固定シード乱数プロバイダー
  - `createTestTickInput()` — テスト用 TickGameStateInput 生成（デフォルト値付き）
  - ※ `applyPlayerMove()`, `applyPlayerAttack()`, `setupStage()` は tickGameState の DI を直接利用する方針のため不要と判断
- [x] **P7-1-2**: `__tests__/builders/gameStateBuilder.ts` は Phase 6 で十分なメソッドが揃っていたため追加不要
  - `withPlayer()`, `withEnemy()`, `withEnemies()`, `withItems()`, `withTraps()`, `withWalls()` は既に実装済み

### P7-2: ゲームエンジン統合テストの実装 ✅

- [x] **P7-2-1**: `__tests__/integration/gameEngine.test.ts` を作成 — 12テスト
- [x] **P7-2-2**: 「敵と接触してダメージを受け、無敵時間中は追加ダメージを受けない」テスト
- [x] **P7-2-3**: 「無敵時間経過後は再びダメージを受ける」テスト
- [x] **P7-2-4**: 「プレイヤー位置のアイテムを拾い、HPが回復する」テスト
- [x] **P7-2-5**: 「レベルアップアイテムを拾うとpendingLevelPointsが増加する」テスト
- [x] **P7-2-6**: 「リジェネが基本間隔経過後に発動する」テスト
- [x] **P7-2-7**: 「HPが最大の場合はリジェネが発動しない」テスト
- [x] **P7-2-8**: 「ダメージ罠を踏むとダメージを受ける」テスト
- [x] **P7-2-9**: 「スロー罠を踏むと移動速度が低下する」テスト
- [x] **P7-2-10**: 「テレポート罠を踏むと位置が変わる」テスト
- [x] **P7-2-11**: 「HPが0になるとゲームオーバーになる」テスト
- [x] **P7-2-12**: 「ダメージを受けた後にアイテムで回復し、リジェネも発動する」複合テスト
- [x] **P7-2-13**: 「鍵アイテムを拾ってhasKeyフラグが立つ」テスト

### P7-3: 決定的シナリオテストの実装 ✅

- [x] **P7-3-1**: `__tests__/scenarios/stagePlaythrough.test.ts` を作成 — 4テスト
- [x] **P7-3-2**: 「固定シードで同一入力なら同一結果になる（決定性の検証）」テスト
- [x] **P7-3-3**: 「連続ティックでダメージ→回復→罠の一連のフローが決定的に実行される」テスト
- [x] **P7-3-4**: 「ステージ間引き継ぎでプレイヤー能力値が維持される」テスト
- [x] **P7-3-5**: 「複数の罠種類を順番に踏むシナリオが正しく動作する」テスト（クールダウン検証含む）

### P7-4: 画面遷移フックテストの実装 ✅

- [x] **P7-4-1**: `__tests__/integration/screenTransition.test.ts` を作成 — 6テスト
- [x] **P7-4-2**: 「TITLE → CLASS_SELECT → PROLOGUE → GAME の正常フロー」テスト
- [x] **P7-4-3**: 「盗賊を選択してもGAMEまで正しく遷移する」テスト
- [x] **P7-4-4**: 「GAME → GAME_OVER → タイトルに戻れる（死亡フロー）」テスト
- [x] **P7-4-5**: 「ゲームオーバーからリトライできる」テスト
- [x] **P7-4-6**: 「STAGE_CLEAR → STAGE_STORY → STAGE_REWARD → GAME のステージ進行」テスト
- [x] **P7-4-7**: 「最終ステージクリア後にFINAL_CLEARになりタイトルに戻れる」テスト

### P7-5: Phase 7 完了確認 ✅

- [x] **P7-5-1**: 統合テストが 5 シナリオ以上実装されていることを確認 — 12テスト実装
- [x] **P7-5-2**: 決定的シナリオテストが 3 シナリオ以上実装されていることを確認 — 4テスト実装
- [x] **P7-5-3**: 画面遷移テストが 4 シナリオ以上実装されていることを確認 — 6テスト実装
- [x] **P7-5-4**: 全テストが安定して通ることを確認 — 357スイート / 4685テスト全パス
- [x] **P7-5-5**: `npx tsc --noEmit` が通ることを確認
- [x] **P7-5-6**: レビュー実施・未使用インポート削除・playerRef の useRef 化
...
- [x] **F-1-1**: `npm run lint:ci` が通ることを確認 — ESLint エラー 0（未使用インポート 8 件を修正）
- [x] **F-1-2**: `npm run typecheck` が通ることを確認 — `npx tsc --noEmit` 通過
- [x] **F-1-3**: `npm test` が通ることを確認 — 357スイート / 4685テスト全パス
- [x] **F-1-4**: `npm run build` が通ることを確認 — webpack ビルド成功（既存 warning 2 件のみ）
- [x] **F-1-5**: 統合テスト・シナリオテストが全て通ることを確認 — 統合 12 + シナリオ 4 + 画面遷移 6 = 22 テスト全パス
- [ ] **F-1-6**: ゲームが正常に動作することを手動で確認 — ※ユーザーによる手動確認が必要

### F-2: コードメトリクス確認 ✅

- [x] **F-2-1**: ルート直下のドメインファイルが 0 であることを確認 — `index.ts`, `types.ts` のみ残存
- [x] **F-2-2**: ファイル行数の確認
  - 300 行超のファイルは P4-5-1 で記録された既知の例外:
    - `Game.tsx` (990行): Canvas 描画ロジック 670 行が密結合
    - `enemyAiFunctions.ts` (600行): 敵 AI の全戦略を統合
    - sprites ファイル群: データ定義ファイル（ロジックなし）
    - `effectManager.ts` (505行): エフェクト統合管理
  - ロジックファイルは概ね 300 行以内を達成
- [x] **F-2-3**: グローバル可変状態が 0 であることを確認 — domain/application 層に `let .*Counter` なし
- [x] **F-2-4**: テストカバレッジが目標値に達していることを確認
  - domain/entities: 97.65%（目標 90% ✅）
  - domain/services: 94.12%（目標 85% ✅）
  - domain/services/gimmickPlacement: 96.08%（目標 85% ✅）
  - domain/factories: 100% ✅
  - domain/types: 100% ✅

### F-3: ドキュメント更新 ✅

- [x] **F-3-1**: `README.md` のファイル構成を更新 — リファクタリング後のディレクトリ構造に全面改訂
- [x] **F-3-2**: `types.ts` の分割について記載を追加 — barrel re-export の説明を追記

### F-4: Phase 2 レビュー残課題の最終確認 ✅

- [x] **F-4-1**: `shared/index.ts` の役割を整理 — `domain/contracts` の公開 API 用再エクスポート層として機能しており、`ipne/index.ts` 経由で外部に提供。役割明確で維持が適切
- [x] **F-4-2**: domain層から外部レイヤーへの参照が完全に0であることを最終確認 — application, infrastructure, presentation, 外部 utils/assets への参照ゼロを確認済み
...
| Phase 7: 統合・シナリオテスト | 18 (+14 追加テスト) | 中 | ✅ 完了 |
| 最終確認 | 13 (+2 レビュー残課題) | 小 | ✅ 完了 |
| **合計** | **180** | — | **✅ 全完了** |
