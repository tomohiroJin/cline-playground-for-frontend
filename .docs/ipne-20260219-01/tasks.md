...
- **前提**: V-5.2（プレースホルダー差し替え準備の確認）
- **作業内容**:
  - 画像生成 AI を使用して 9 枚のストーリー画像を生成
  - 共通仕様: 960×540 px、WebP（品質 80〜85）、ダークファンタジー基調
  - 画像内にテキストを含めない
  - 各画像の仕様は `images.md` を参照
- **受入条件**:
  - [x] 9 枚の WebP 画像が生成されている
  - [x] 各画像が 960×540 px である
  - [x] ファイルサイズが 50〜150 KB の範囲内
  - [x] 画像内にテキストが含まれていない

### I-6.2: 画像ファイルの配置

- **対象ディレクトリ**: `src/assets/images/`
- **前提**: I-6.1
- **作業内容**:
  - 生成した 9 枚の画像を `src/assets/images/` に配置
  - ファイル名規則: `ipne_story_{用途}.webp`
- **受入条件**:
  - [x] 以下の 9 ファイルが配置されている:
...
### I-6.3: `storyImages.ts` の修正

- **対象ファイル**: `src/features/ipne/storyImages.ts`
- **前提**: I-6.2
- **作業内容**:
  - Canvas API によるプレースホルダー生成を削除
  - 実画像を import 文で読み込み
  - `IMAGE_SOURCES` マッピングで画像キー → import を対応付け
  - `getStoryImage()` の返却値を import 済み画像パスに変更
- **受入条件**:
  - [x] Canvas API によるプレースホルダー生成コードが削除されている
  - [x] 9 枚の画像が import されている
  - [x] `getStoryImage()` が import 済みの画像パスを返す
  - [x] `hasStoryImage()` が正しく動作する
...
- **作業内容**:
  ...
  - Webpack が WebP ファイルを正しくバンドルすることを確認
...
- **前提**: V-6.1
- **作業内容**:
  - プロローグ 3 シーンで画像が表示されることを確認
  - ステージ 1〜5 クリア後のストーリー画面で画像が表示されることを確認
  - 画像のフェードインアニメーションが正しく動作することを確認
  - モバイル表示でレイアウトが崩れないことを確認
- **受入条件**:
  - [x] 全 8 シーン（プロローグ 3 + ステージ 5）で正しい画像が表示される
  - [x] 画像が正しいタイミング・シーンで表示される
  - [x] フェードインアニメーションが動作する
  - [x] モバイル表示で問題なし
...
- **前提**: V-6.1
- **作業内容**:
  - `Prologue.tsx` — 修正不要（`getStoryImage()` 経由）
  - `StageStory.tsx` — 修正不要（同上）
  - `FinalClear.tsx` — 修正不要（`getEndingImage()` は別系統）
  - `story.ts` — 修正不要（`imageKey` 文字列のみ）
- **受入条件**:
  - [x] `storyImages.ts` 以外のファイルに変更が不要であることを確認
...
I-2.5 ─┐
I-2.6 ─┤
I-2.7 ─┼──→ I-5.1 ──→ I-5.2 ──→ V-5.1 ──→ V-5.2
I-3.7 ─┤
I-4.6 ─┘
         │
V-5.2 ──→ I-6.1 ──→ I-6.2 ──→ I-6.3 ──→ V-6.1 ──→ V-6.2
                                                   └──→ V-6.3
```
