...
| 5 | 画像アセット差し替え | プレースホルダー画像を画像生成 AI で作成した本番アセットに差し替え |

**方針**: 画像は全てプレースホルダー（単色+テキスト）で配置し、本番アセットはフェーズ 6 で画像生成 AI を使用して作成・差し替え。画像仕様の詳細は `images.md` を参照。
...
フェーズ5 ── 統合とポリッシュ
  │
フェーズ6 ── 画像アセット差し替え（プレースホルダー → 本番画像）
```
...
フェーズ 1〜5 で Canvas API プレースホルダーとして実装されていたストーリー画像を、画像生成 AI で作成した本番アセットに差し替える。

### 前提

- フェーズ 5 完了（プレースホルダー差し替え準備の確認済み）
- 画像仕様書 (`images.md`) の作成完了

### 変更対象

| ファイル | 変更内容 |
|---------|---------|
| `src/assets/images/ipne_story_*.webp` (新規 9 ファイル) | 本番画像アセットの配置 |
| `src/features/ipne/storyImages.ts` | Canvas プレースホルダー → import 実画像 |

### 作業内容

#### 6-1. 画像生成

画像生成 AI を使用して 9 枚のストーリー画像を生成する。仕様の詳細は `images.md` を参照。

**共通仕様**:
- 出力サイズ: 960×540 px（Retina 対応、表示は 480×270）
- 出力形式: WebP（品質 80〜85、50〜150 KB）
- 画風: アニメ調コンセプトアート、ダークファンタジー基調
- 人物は登場させず、構造物・風景を中心に描写
- 画像内にテキストを一切含めない
...
| # | ファイル名 | シーン |
|---|-----------|--------|
| 1 | `ipne_story_prologue_1.webp` | 任務ブリーフィング |
| 2 | `ipne_story_prologue_2.webp` | ダンジョン入口 |
| 3 | `ipne_story_prologue_3.webp` | 閉じた入口 |
| 4 | `ipne_story_stage_1.webp` | 第一層突破 |
| 5 | `ipne_story_stage_2.webp` | 深部への接近 |
| 6 | `ipne_story_stage_3.webp` | 異変 |
| 7 | `ipne_story_stage_4.webp` | 最深部へ |
| 8 | `ipne_story_stage_5.webp` | 封鎖解除 |
| 9 | `ipne_story_game_over.webp` | 冒険の終わり |

#### 6-2. ファイル配置

生成した画像を `src/assets/images/` に配置する。

#### 6-3. `storyImages.ts` の修正

Canvas プレースホルダー生成を import 実画像に差し替える。

```typescript
// Canvas プレースホルダーコードを削除し、以下に差し替え
...
**変更不要な箇所**: `Prologue.tsx`, `StageStory.tsx`, `FinalClear.tsx`, `story.ts` — `getStoryImage()` 経由で取得するため修正不要。

### 完了条件

- 9 枚の本番画像が `src/assets/images/` に配置されている
- `storyImages.ts` が import 実画像を返すように修正されている
- `npm run build` が成功する
- ブラウザで全画面遷移を目視確認し、正しい画像が表示される
...
| プレースホルダー画像の見栄え | 低 | Canvas 動的生成で最低限の表示品質を確保。フェーズ 6 で本番画像に差し替え |
| 画像生成 AI の品質ばらつき | 中 | `images.md` に詳細な仕様を記載。再生成可能な構造で設計 |
...
| フェーズ 6 | フェーズ 5 | 画像生成 AI による本番アセット作成・差し替え |

フェーズ 2, 3, 4 は並列実施可能。フェーズ 6 の画像生成はフェーズ 5 完了後に実施。
