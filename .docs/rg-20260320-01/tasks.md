...
> ステータス: フェーズ4 完了、フェーズ5 進行中（5-4, 5-5 完了）
...
- [ ] スモークテスト全パス（1-5 保留）
- [x] `npm run build` 成功
- [x] ドメイン層テストカバレッジ 90% 以上（Stmts 96.21%）
...
- [x] `npm run typecheck` パス
- [x] `npm run lint` パス（新モジュール対象）
- [x] `npm test` 全パス（27スイート、245テスト）
...
- [ ] 未使用のインポート・ファイルの最終確認
- [ ] 全テスト実行・パス確認
    > **方針**: 旧ファイルは RacingGame.tsx が直接参照しているため、5-1 の RacingGame.tsx
    > リファクタリング完了後に削除する。index.ts は新旧両方を re-export して後方互換を維持。
...
- [x] カバレッジ確認（ドメイン層: Stmts 96.21% / Branch 90.4% / Lines 96.96%）
  - [ ] ドメイン層: 90% 以上
  - [ ] 全体: 80% 以上
...
- [x] TypeScript `any` 使用数: 0個（新モジュール対象）
- [x] `npm run typecheck` パス
- [x] `npm run lint` パス（新モジュール対象）
...
- [x] ドメイン層テストカバレッジ 90% 以上（Stmts 96.21%）
- [ ] 全体テストカバレッジ 80% 以上
- [x] 統合テストシナリオ 10 以上（GameOrchestrator 12テスト + 統合テスト 20テスト）

### アーキテクチャ

- [x] `RacingGameNew.tsx` が 200行以下（196行）
- [x] ドメイン層が外部依存ゼロ
- [ ] 最大ファイル行数が 300行以下（旧 renderer.ts 682行、旧 RacingGame.tsx 758行が残存）
- [x] 全ポートインターフェースが適切に実装されている
