...
- [ ] **2.5-1-1**: `use-test-mode.ts` フック作成（キー入力バッファリング、「jinjinjin」検出）
- [ ] **2.5-1-2**: idle状態でのみ検知するよう制御（プレイ中のキー入力は無視）
- [ ] **2.5-1-3**: トグル動作（再度「jin」×3でテストモード無効化）
- [ ] **2.5-1-4**: `useTestMode` のユニットテスト

### 2.5-2. テストモードインジケーター 🟡

- [ ] **2.5-2-1**: 画面右上に「TEST」バッジ表示（半透明、テストモード時のみ）
- [ ] **2.5-2-2**: `FalldownShooterGame.tsx` に `useTestMode` フック統合

### 2.5-3. デバッグパネル — グリッド操作 🔴

- [ ] **2.5-3-1**: `TestModePanel.tsx` コンポーネント作成（折りたたみ可能なパネル）
- [ ] **2.5-3-2**: 「N行セット」ボタン実装（1〜4行、プレイヤーX列だけ空けてグリッド最下段付近を埋める）
- [ ] **2.5-3-3**: 「グリッドクリア」ボタン実装
- [ ] **2.5-3-4**: グリッド操作のユニットテスト

### 2.5-4. デバッグパネル — エフェクト操作 🟡

- [ ] **2.5-4-1**: 各種シェイク手動発火ボタン実装（Bomb / Blast / Line / G.Over）
- [ ] **2.5-4-2**: ハイスコア演出手動発火ボタン実装

### 2.5-5. デバッグパネル — スコア・ステージ操作 🟡

- [ ] **2.5-5-1**: スコア加算ボタン実装（+1000）
- [ ] **2.5-5-2**: スキルゲージ満タンボタン実装
- [ ] **2.5-5-3**: ステージスキップボタン実装

### 2.5-6. デバッグパネル — 状態表示 🟢

- [ ] **2.5-6-1**: コンボ数・倍率・スキルゲージ・スコア・ステージのリアルタイム表示
