...
- [x] プロフィールカード閉じるフェードアウトアニメーション
  - [x] `isClosing` 状態 + `startClose()` で遅延実行（200ms）
  - [x] Overlay: `fadeOut` アニメーション、CardContainer: `slideDown` アニメーション
  - [x] 閉じるボタン / 背景タップ / Escape すべてフェードアウト対応
- [x] club フィールドの表示追加
  - [x] 基本情報セクションに `{profile.school} {profile.club}` を表示
