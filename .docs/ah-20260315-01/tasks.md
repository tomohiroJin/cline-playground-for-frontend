...
| P2-08 テスト・動作確認 | [x] 完了 | ✅ ※3 | 2026-03-16 |
...
### コンポーネント作成
- [x] `components/CharacterProfileCard.tsx` を新規作成
- [x] Props 型定義

### 表示要素
- [x] モーダルオーバーレイ（背景タップで閉じる）
- [x] 立ち絵表示（中央配置、最大幅 200px、最大高さ 300px）
- [x] 立ち絵タップで表情切替（normal ⇔ happy、クロスフェード 150ms）
- [x] 「タップで表情変更」ヒントテキスト
- [x] `portrait` 未定義時のアイコン拡大フォールバック
- [x] 代表セリフ（イタリック、テーマカラー、引用マーク）
- [x] キャラ名（太字 24px）+ 読み（14px、グレー）
- [x] 基本情報（学年・年齢・身長・誕生日・所属 — 「|」区切り）
- [x] 性格キーワードのタグ表示（角丸チップ、テーマカラー）
- [x] プレイスタイル名 + 得意技名 + 説明
- [x] キャラクター紹介文
- [x] 閉じるボタン（✕、カード右上）

### アニメーション
- [x] オーバーレイのフェードイン（200ms）
- [x] カードのスライドアップ + フェードイン（300ms、ease-out）
- [ ] 閉じる時のフェードアウト（200ms）
...
### テスト
- [x] `CharacterProfileCard.test.tsx` を作成
  - [x] プロフィール情報が正しく表示されるテスト
  - [x] 立ち絵が正しく表示されるテスト
  - [x] 立ち絵タップで表情が切り替わるテスト
  - [x] `portrait` 未定義時にアイコンが表示されるテスト
  - [x] 閉じるボタンで `onClose` が呼ばれるテスト
  - [x] 背景タップで `onClose` が呼ばれるテスト
  - [x] 性格タグが正しく表示されるテスト

### 残課題（レビュー指摘 — 後日対応）
- [ ] `club`（部活名）フィールドを基本情報セクションに表示するか検討（現在未表示）
- [ ] 閉じる時のフェードアウトアニメーション（200ms）の実装（状態管理が複雑化するため P2-07 統合時に対応を検討）
- [ ] `PersonalityTag` の背景色を 8桁 HEX（`${color}20`）から `rgba()` 形式に変更するか検討（古いブラウザ対応が必要な場合）
- [ ] 異なるキャラクター切替時のスクロール位置リセット対応（P2-07 統合時に確認）
...
### キャラ表情差分の追加
- [x] `ResultScreen.tsx` に `cpuCharacter?`, `playerCharacter?` Props を追加
- [x] ストーリーモード勝利時: プレイヤー立ち絵（happy）+ 対戦キャラ立ち絵（normal）を表示
- [x] ストーリーモード敗北時: プレイヤー立ち絵（normal）+ 対戦キャラ立ち絵（happy）を表示
- [x] フリー対戦時: 対戦キャラ立ち絵を表示（`portrait` がある場合のみ）
- [x] 立ち絵未定義時のフォールバック（表示しない）
- [x] 立ち絵のフェードインアニメーション（300ms）

### アンロック通知の追加
- [x] `newlyUnlockedCharacterName?` Props を追加
- [x] 通知バナー: 金色ボーダー + 🔓アイコン + 「{キャラ名} が図鑑に追加されました！」
- [x] 通知表示タイミング: スコアカウントアップ完了後に 500ms ディレイでフェードイン

### AirHockeyGame 統合
- [x] `AirHockeyGame.tsx` から `ResultScreen` に `cpuCharacter`, `playerCharacter` を渡す
- [x] ストーリークリア時の図鑑アンロック判定を追加
- [x] 新規アンロックキャラ名を `ResultScreen` に渡す

### テスト
- [x] `ResultScreen.test.tsx` のテスト更新
  - [x] 勝利時にプレイヤー happy + 対戦キャラ normal が表示されるテスト
  - [x] 敗北時にプレイヤー normal + 対戦キャラ happy が表示されるテスト
  - [x] キャラ情報なし時に立ち絵が表示されないテスト（後方互換性）
  - [x] アンロック通知が表示されるテスト
  - [x] アンロック通知なし時にバナーが表示されないテスト
...
### ボタン追加
- [x] `TitleScreen.tsx` に `onCharacterDexClick` Props を追加
- [x] `TitleScreen.tsx` に `newUnlockCount` Props を追加
- [x] 「キャラクター」ボタンの追加（ストーリーボタンの下に配置）
- [x] ボタンスタイル: 紫系グラデーション（#9b59b6 ベース）
- [x] 通知バッジ: 赤丸（直径 20px）+ 白文字数字、`newUnlockCount > 0` 時のみ表示
- [x] バッジのパルスアニメーション（1.5秒周期）
- [x] メニューレイアウトの調整（ボタン増加に伴うスペース調整）

### テスト
- [x] `TitleScreen.test.tsx` のテスト更新
  - [x] 「キャラクター」ボタンが表示されるテスト
  - [x] ボタンタップで `onCharacterDexClick` が呼ばれるテスト
  - [x] `newUnlockCount > 0` 時にバッジが表示されるテスト
  - [x] `newUnlockCount === 0` 時にバッジが表示されないテスト
...
### 既存テスト
- [x] `npm test` で全テストパス（410 suites, 5269 tests）
- [x] `tsc --noEmit` で型エラーなし

### 新規テスト一覧
- [x] `dex-data.test.ts`: 図鑑データの整合性テスト
  - [x] 全8キャラクターのエントリが存在するテスト
  - [x] `characterId` が `characters.ts` の ID と一致するテスト
  - [x] アンロック条件の `stageId` が `dialogue-data.ts` のステージ ID と一致するテスト
  - [x] 必須フィールドがすべて入力されているテスト
- [x] `dex.test.ts`: アンロックロジックのテスト（P2-02 で作成）
- [x] `useCharacterDex.test.ts`: カスタムフックのテスト（P2-02 で作成）
- [x] `CharacterDexScreen.test.tsx`: 図鑑画面のテスト（P2-03 で作成）
- [x] `CharacterProfileCard.test.tsx`: プロフィールカードのテスト（P2-04 で作成）

### 動作確認（ユニット/統合テストでカバー）
- [x] タイトル画面 → 「キャラクター」ボタン → 図鑑画面の遷移（AirHockeyGame.test.tsx）
- [x] 図鑑画面のカード一覧表示（アンロック/ロック状態）（CharacterDexScreen.test.tsx + AirHockeyGame.test.tsx）
- [x] カードタップ → プロフィールカード表示（CharacterDexScreen.test.tsx + AirHockeyGame.test.tsx）
- [x] プロフィールカードの立ち絵表情切替（CharacterProfileCard.test.tsx）
- [x] プロフィールカードを閉じる（✕ / 背景タップ / Escape）（CharacterProfileCard.test.tsx）
- [x] 「戻る」ボタン → タイトル画面に戻る（AirHockeyGame.test.tsx）
- [x] ストーリーモード 1-1 クリア → ヒロがアンロック → リザルト画面に通知表示（dex.test.ts + ResultScreen.test.tsx）
- [x] ストーリーモード 1-2 クリア → ミサキがアンロック（dex.test.ts）
- [x] ストーリーモード 1-3 クリア → タクマがアンロック（dex.test.ts）
- [x] リザルト画面でキャラ表情差分が正しく表示される（勝利/敗北両方）（ResultScreen.test.tsx）
- [x] タイトル画面の通知バッジが表示/消去される（TitleScreen.test.tsx）
- [x] 初回プレイ時（全ロック状態）の動作確認（AirHockeyGame.test.tsx — ヒロ/ミサキ/タクマが「???」表示）
- [x] 全クリア状態（全アンロック）の動作確認（useCharacterDex.test.ts — completionRate テスト）
- [x] フリー対戦に影響がないことの確認（AirHockeyGame.test.tsx）
- [x] ストーリーモードの既存フローに影響がないことの確認（全既存テストパス）

### パフォーマンス確認
- [x] 図鑑画面の画像読み込みが適切（遅延読み込み動作）— コードレビューで確認
- [x] プロフィールカード表示時のアニメーションが滑らか — CSS アニメーション定義確認済み
- [x] localStorage の読み書きが正常 — dex.test.ts でカバー

### レビュー・リファクタリング（P2-08 実施）
- [x] CharacterDexScreen.tsx: `useEffect` 依存配列の ESLint 無視を `useRef` パターンに改善
- [x] CharacterDexScreen.tsx: デフォルトボーダーカラーを定数化（`DEFAULT_BORDER_COLOR`）
- [x] CharacterProfileCard.tsx: `handlePortraitClick` を `useCallback` でメモ化
...
- [x] 全チェックボックスが完了
- [x] 既存テスト + 新規テストが全パス
- [x] タイトル → 図鑑 → プロフィール の遷移が正常動作
- [x] ストーリークリア時のアンロックが正常動作
- [x] リザルト画面の表情差分が正常動作
- [x] フリーモード・ストーリーモードの既存動作に影響なし
