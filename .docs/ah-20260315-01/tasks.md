...
### コンポーネント作成
- [ ] `components/CharacterDexScreen.tsx` を新規作成
- [ ] Props 型定義

### ヘッダー部
- [ ] 「キャラクター図鑑」タイトルの表示
- [ ] 「戻る」ボタンの実装（`onBack` コールバック）
- [ ] コンプリート率のプログレスバー表示（例: 5/8）

### キャラクターカードグリッド
- [ ] 2列グリッドレイアウトの実装
- [ ] アンロック済みカード: アイコン（64x64）+ 名前 + テーマカラーボーダー
- [ ] ロック中カード: グレースケール + シルエット + 「???」表示
- [ ] NEW バッジ: 赤背景 + 白文字、カード右上に配置
- [ ] カードタップでプロフィール表示（`onSelectCharacter` コールバック）
- [ ] ロック中カードはタップ不可

### アニメーション
- [ ] カード一覧の stagger フェードイン（各50ms遅延）
- [ ] NEW バッジのパルスアニメーション（1秒周期）
- [ ] カードタップ時のスケールダウン（0.95、100ms）

### テスト
- [ ] `CharacterDexScreen.test.tsx` を作成
  - [ ] 全キャラカードが表示されるテスト
  - [ ] アンロック済みキャラのアイコンと名前が表示されるテスト
  - [ ] ロック中キャラが「???」で表示されるテスト
  - [ ] NEW バッジが新規アンロックキャラに表示されるテスト
  - [ ] アンロック済みカードタップで `onSelectCharacter` が呼ばれるテスト
  - [ ] ロック中カードタップで `onSelectCharacter` が呼ばれないテスト
  - [ ] コンプリート率が正しく表示されるテスト
  - [ ] 「戻る」ボタンで `onBack` が呼ばれるテスト
