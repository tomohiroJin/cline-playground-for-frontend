...
- **受入条件**:
  - [ ] ファクトリ関数が TypeScript コンパイルに通る
  - [ ] 各ファクトリが型安全にデフォルト値を提供する
  - [ ] overrides パラメータで部分的な上書きが可能
...
- **受入条件**:
  - [ ] `npm run test:e2e` でスモークテストが通る
  - [ ] Page Object Model が基本操作 + `injectSeededRng` をカバー
  - [ ] Playwright 設定が labyrinth-echo テストを認識する
...
- **受入条件**:
  - [ ] ディレクトリ構造が計画通り作成されている
  - [ ] `invariant` 関数が新しい場所からエクスポートされている
  - [ ] TypeScript コンパイルが通る
...
- **受入条件**:
  - [ ] `any` 型が domain 層に存在しない
  - [ ] `eslint-disable` が domain 層に存在しない
  - [ ] 全モデルファイルが TypeScript コンパイルに通る
  - [ ] 既存テストに影響がない（旧ファイルからの re-export で互換性維持）
...
- **受入条件**:
  - [ ] 全モデルのファクトリ関数がテストされている
  - [ ] 不変条件違反時のエラーがテストされている
  - [ ] カバレッジ 90% 以上
...
- **受入条件**:
  - [ ] 新しい条件型で全既存条件パターンを表現可能
  - [ ] `parseCondition` が全既存条件文字列を正しくパースする
  - [ ] 既存の `evalCond` 呼び出しが互換ラッパー経由で動作する
  - [ ] TypeScript の網羅性チェック（exhaustive switch）が有効
...
- **作業内容**:
  ...
  - barrel export を作成
- **受入条件**:
  - [ ] 全定数が `domain/constants/` に移動されている
  - [ ] 新しい型定義と整合している
  - [ ] 旧ファイルからの re-export で互換性維持
  - [ ] TypeScript コンパイルが通る
...
- **受入条件**:
  - [ ] 全ドメインサービスが純粋関数で実装されている
  - [ ] DbC アサーションが事前条件・事後条件に設定されている
  - [ ] 旧 `game-logic.ts` からの re-export で互換性維持
  - [ ] TypeScript コンパイルが通る
...
- **受入条件**:
  - [ ] `SeededRandomSource` が同一 seed で同一の乱数列を返す
  - [ ] `shuffleWith` が `RandomSource` を受け取り、決定論的にシャッフル可能
  - [ ] 既存の `shuffle` 呼び出しが互換で動作する
  - [ ] TypeScript コンパイルが通る
...
- **作業内容**:
  ...
  - `validateEvents` を移動
- **受入条件**:
  - [ ] イベント関連の型定義が domain 層に統合されている
  - [ ] `pickEvent` が `RandomSource` を受け取り、決定論的テストが可能
  - [ ] `RandomSource` を渡さない場合は `Math.random` を使用（後方互換）
  - [ ] 旧ファイルからの re-export で互換性維持
...
- **受入条件**:
  - [ ] インターフェースが domain 層の型のみに依存している
  - [ ] infrastructure 層への依存がない（DIP）
  - [ ] `RandomPort` が定義されている
  - [ ] TypeScript コンパイルが通る
...
- **作業内容**:
  ...
  - 全ユースケースを純粋関数として実装
- **受入条件**:
  - [ ] 全ユースケースが純粋関数で実装されている
  - [ ] ポートインターフェースにのみ依存している
  - [ ] 入出力の型が明確に定義されている
  - [ ] TypeScript コンパイルが通る
