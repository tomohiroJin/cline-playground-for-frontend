...
- **テスト項目**:
  - [ ] 型定義が正しくコンパイルされること
  - [ ] 既存コードの型エラーが発生しないこと（`Player` 拡張の後方互換性）
...
- **テスト項目**:
  - [ ] 定数値が spec.md の仕様通りであること
  - [ ] 既存の `Config`, `Colors`, `Options`, `Courses` と名前衝突がないこと
...
- **テスト項目**:
  - [ ] `speed < DRIFT.MIN_SPEED (0.4)` でドリフトが開始されないこと
  - [ ] ドリフト中の角速度が `Config.game.turnRate × DRIFT.ANGLE_MULTIPLIER (1.8)` になること
  - [ ] ドリフト終了時にブースト量が `DRIFT.BOOST_BASE + duration × DRIFT.BOOST_PER_SEC` で計算されること
  - [ ] ブースト量が `DRIFT.BOOST_MAX (0.3)` を超えないこと
  - [ ] 壁接触（`onTrack === false`）でドリフトが終了すること
...
- **内容**:
  ...
  - ドリフトブーストの速度反映
- **テスト項目**:
  - [ ] ハンドブレーキ押下 + ステアリング入力でドリフトが開始されること
  - [ ] ドリフト中の移動計算が正しいこと
  - [ ] ドリフト終了後にブーストが適用されること
  - [ ] 既存の通常移動に影響がないこと（ハンドブレーキ未押下時）
...
- **テスト項目**:
  - [ ] ドリフト中にスモークパーティクルが生成されること（`Config.game.maxParticles` 内）
  - [ ] ドリフト音が開始/停止されること
  - [ ] ブースト発動時に効果音が再生されること
...
- **テスト項目**:
  - [ ] 壁距離が `trackWidth - HEAT.WALL_THRESHOLD` より大きい（壁に近い）時にゲージが蓄積されること
  - [ ] 車間距離 < `HEAT.CAR_THRESHOLD (40)` かつ > `Config.game.collisionDist (25)` でゲージ蓄積
  - [ ] 距離が近いほど蓄積量が大きいこと（距離反比例）
  - [ ] ゲージが `HEAT.DECAY_RATE` で自然減衰すること
  - [ ] ゲージ 1.0 でブーストが発動し、ゲージが 0 にリセットされること
  - [ ] クールダウン中（`HEAT.COOLDOWN` 秒）にゲージが蓄積されないこと
...
- **内容**:
  ...
  - HEAT ブースト発動の速度反映
- **テスト項目**:
  - [ ] ゲームループ中に HEAT が正しく計算されること
  - [ ] ブースト発動で速度が増加すること
  - [ ] 既存の `Logic.handleCollision()` に影響がないこと
...
- **テスト項目**:
  - [ ] ゲージバーが `HeatState.gauge` に応じた幅で描画されること
  - [ ] 色が青(0〜0.3)→黄(0.3〜0.7)→赤(0.7〜1.0)と遷移すること
  - [ ] ゲージ 1.0 到達時に白点滅すること
...
- **テスト項目**:
  - [ ] 壁接触時に完全停止しないこと（`speed > 0`）
  - [ ] スライド方向がトラックセグメントに沿っていること
  - [ ] `wallStuck` 増加に応じて減速率が変化すること（3段階）
  - [ ] ワープしきい値 15 で正常にワープすること
  - [ ] コースショートカットが不可能なこと（`Logic.updateCheckpoints()` で担保）
...
- **テスト項目**:
  - [ ] 各コースの効果パラメータが spec.md の仕様通りであること
  - [ ] 摩擦係数が正しく適用されること
  - [ ] `segmentBased === true` のコースでセグメントごとに効果が異なること
...
- **テスト項目**:
  - [ ] Forest: 特定セグメントで `frictionMultiplier = 0.85` が適用されること
  - [ ] City: 全体で `frictionMultiplier = 0.90`、`driftAngleBonus = +10%`
  - [ ] Mountain: セグメント傾斜で速度が ±5% 変化すること
  - [ ] Beach: トラック外縁20%で `frictionMultiplier = 0.70`
  - [ ] Night: ビネットエフェクトが描画されること
  - [ ] Snow: `frictionMultiplier = 0.75`、`driftAngleBonus = +20%`
...
- **テスト項目**:
  - [ ] 各コースで対応する視覚エフェクトが表示されること
  - [ ] パーティクル数が `Config.game.maxParticles` を超えないこと
  - [ ] 60fps を維持できるパフォーマンスであること
...
- **テスト項目**:
  - [ ] 6コース（`Courses[0-5]`）全てで正常にレースが完走できること
  - [ ] 2P モード（`mode === '2p'`）で両プレイヤーが正常に動作すること
  - [ ] CPU がドリフト/HEAT を使用すること（`skill` パラメータに応じて）
  - [ ] フレームレートが 55fps 以上を維持すること
  - [ ] 既存のベストタイム保存（`bests`）に影響がないこと
  - [ ] デモモード（`useIdle`）が正常に動作すること
...
- **テスト項目**:
  - [ ] 15枚全てのカードデータが正しく定義されていること
  - [ ] カテゴリ分類: speed 5枚, handling 4枚, defense 3枚, special 3枚
  - [ ] レアリティ分布: R 8枚, SR 5枚, SSR 2枚
...
- **テスト項目**:
  - [ ] デッキが正しくシャッフルされること
  - [ ] 3枚ドローでレアリティ確率が概ね正しいこと（R:60%, SR:30%, SSR:10%）
  - [ ] 選択したカードがデッキから除外されること
  - [ ] デッキ3枚未満で全15枚から再生成されること
  - [ ] 効果の加算合算が正しいこと（乗算ではなく加算）
...
- **テスト項目**:
  - [ ] 各カード効果が正しく適用されること（15枚すべて）
  - [ ] 効果が1ラップで正しく解除されること
  - [ ] シールドが衝突時に消費されること（`Logic.handleCollision()` 連携）
  - [ ] ワイルドカードがランダムに2枚効果を発動すること
  - [ ] 複数カード効果の加算合算が正しいこと
...
- [ ] **難易度**: L
- **依存関係**: T-102
- **変更ファイル**: `components/DraftCardUI.tsx`（新規作成）
- **内容**:
...
- **テスト項目**:
  - [ ] 3枚のカードが正しく表示されること
  - [ ] キー操作で選択・決定ができること
  - [ ] 15秒でタイムアウトしてランダム選択されること
  - [ ] 2P モードで P1→P2 の順に選択できること
  - [ ] アニメーションが滑らかに動作すること
...
- **内容**:
  ...
  - デモモード時はドラフトスキップ
- **テスト項目**:
  - [ ] ラップ完了時に `state === 'draft'` に遷移すること（最終ラップ除く）
  - [ ] 最終ラップ完了時は直接 `state === 'result'` に遷移すること
  - [ ] カード選択後に `state === 'race'` に戻りレースが再開されること
  - [ ] CPU が自動でカードを選択すること
  - [ ] `Options.laps === 1` の設定時はドラフトが発生しないこと
  - [ ] デモモード時にドラフト UI が表示されないこと
...
- **テスト項目**:
  - [ ] 3フレームに1回の頻度で記録されること（フレームカウンタ % 3 === 0）
  - [ ] `state === 'draft'` 中は記録が停止されること
  - [ ] `GhostFrame` のフォーマットが正しいこと（x, y, angle, speed, lap, t）
...
- **テスト項目**:
  - [ ] フレーム間の線形補間が正しいこと
  - [ ] 角度の補間が最短経路で行われること（-PI〜PI を考慮）
  - [ ] データ範囲外の時間（レース終了後）でエラーが発生しないこと
...
- **テスト項目**:
  - [ ] 保存・読込が正しく動作すること（JSON シリアライズ/デシリアライズ）
  - [ ] ベストタイム更新時のみ上書きされること
  - [ ] データが存在しない場合に `null` が返ること
  - [ ] localStorage 容量超過時にエラーハンドリングされること（try-catch）

### T-109: ゴースト描画・トグル UI

- [ ] **難易度**: M
- **依存関係**: T-107, T-108
- **変更ファイル**: `renderer.ts`, `components/GhostToggle.tsx`（新規作成）, `RacingGame.tsx`
- **内容**:
  - `renderer.ts`: `Render.kart()` を活用したゴースト車体の半透明描画（`globalAlpha = 0.3`）、"GHOST" ラベル
  - `GhostToggle.tsx`: ON/OFF トグルコンポーネント（`VolumeCtrl` スタイル踏襲）
  - `RacingGame.tsx`: ゲームループにゴースト再生を統合、`Logic.handleCollision()` 対象外に設定
- **テスト項目**:
  - [ ] ゴースト車体が半透明（alpha 0.3）で描画されること
  - [ ] トグル ON/OFF でゴースト表示が切り替わること
  - [ ] ゴーストデータがない場合はトグルがグレーアウトすること
  - [ ] ゴースト車体が `Logic.handleCollision()` で衝突判定されないこと
...
- **テスト項目**:
  - [ ] ドリフト 1.5秒以上でボーナスが検出されること（スコア = duration × 100）
  - [ ] HEAT ブースト発動が検出されること（スコア = 150）
  - [ ] 壁距離 10px 未満 0.5秒以上でニアミスが検出されること
  - [ ] `Player.progress` による順位逆転が検出されること（スコア = 300）
  - [ ] `Player.lapTimes` の最速更新でファステストラップが検出されること（スコア = 200）
  - [ ] ゴール時タイム差 0.5秒未満でフォトフィニッシュが検出されること（スコア = 500）
...
- **テスト項目**:
  - [ ] 通知バナーが正しいタイミングで表示されること（2秒間）
  - [ ] 複数通知がキュー順に表示されること
  - [ ] リザルト画面にハイライトサマリーが表示されること（種別×回数+スコア）
  - [ ] 各イベントの背景色が spec.md 通りであること
...
- **テスト項目**:
  - [ ] 3周レース（`Options.laps = 3`）でドラフトが2回発生すること
  - [ ] 5周レース（`Options.laps = 5`）でドラフトが4回発生すること
  - [ ] カード効果がドリフト/HEAT と正しく連動すること（`driftBoostMultiplier`, `heatGainMultiplier`）
  - [ ] ゴーストとハイライトが同時に正常動作すること
  - [ ] 全6コース（`Courses[0-5]`）で完走できること
  - [ ] CPU 対戦（`mode === 'cpu'`）・2P 対戦（`mode === '2p'`）が正常に動作すること
  - [ ] デモモード（`useIdle` 起動）が正常に動作すること
  - [ ] フレームレートが 55fps 以上を維持すること
  - [ ] localStorage ゴーストデータ容量が想定範囲内（1件 ~240KB）であること
