...
- [x] **T-02.10** `[A]` 目視テスト: 全タイル・キャラクター・アイテムが正しく描画されることを確認
...
## フェーズ 4: 移動スピードエフェクト ※ BF-02 により視覚エフェクト削除

> 前提: フェーズ 3 完了 | ストリーム: B
> **注記**: 残像が横並び・前方に表示されるためエフェクトとして機能せず、BF-02 により視覚表現を削除。SPEED_BOOST 効果音は残存。

- [x] **T-04.1** `[B]` `movement.ts` に `SPEED_EFFECT_THRESHOLD = 5.2` 定数追加 ※ BF-02 により削除予定
- [x] **T-04.2** `[B]` `effects/speedEffect.ts` — 残像管理（過去 3 フレーム位置保持） ※ BF-02 により削除予定
- [x] **T-04.3** `[B]` `effects/speedEffect.ts` — スピードライン描画（4 本、移動逆方向） ※ BF-02 により削除予定
- [x] **T-04.4** `[B]` Game.tsx にスピードエフェクト描画統合 ※ BF-02 により削除予定
- [x] **T-04.5** `[B]` 目視テスト: 盗賊 Lv3 以上で残像・スピードラインが表示されることを確認 ※ BF-02 により削除予定
...
- [x] **T-05.1** `[B]` `types.ts` に `ScreenState.DYING = 'dying'` 追加
- [x] **T-05.2** `[B]` `effects/deathEffect.ts` — 死亡アニメーション管理クラス
- [x] **T-05.3** `[B]` 死亡フェーズ 1 実装: 点滅（0.0～0.5 秒）
- [x] **T-05.4** `[B]` 死亡フェーズ 2 実装: 赤変色（0.5～1.0 秒）
- [x] **T-05.5** `[B]` 死亡フェーズ 3 実装: パーティクル分解（1.0～1.5 秒）
- [x] **T-05.6** `[B]` `useGameLoop.ts` — GAME_OVER → DYING 遷移変更、1.5 秒後に GAME_OVER 遷移
- [x] **T-05.7** `[B]` Game.tsx — DYING 状態中の描画処理（死亡アニメーション表示）
- [x] **T-05.8** `[B]` 目視テスト: HP 0 で 1.5 秒のアニメーション後にゲームオーバー画面に遷移することを確認
...
- [x] **T-06.12** `[C]` 目視テスト: 全 12 種の効果音が適切なタイミングで鳴ることを確認
...
- [x] **BF-02** `[B]` スピードエフェクト視覚表現の削除（残像が横並び・前方に表示される不具合のため）
  - [x] BF-02.1: `speedEffect.ts` を削除
  - [x] BF-02.2: `Game.tsx` からスピードエフェクト描画の呼び出しを除去
  - [x] BF-02.3: `movement.ts` の `SPEED_EFFECT_THRESHOLD` 定数を削除
  - [x] BF-02.4: SPEED_BOOST 効果音の定義は残存させる（将来別用途の可能性）

---

## 最終検証

- [x] **T-99.1** `npm run build` 成功確認
- [x] **T-99.2** `npm test` 全テストパス確認
- [x] **T-99.3** `npm run dev` で全機能統合動作確認
- [x] **T-99.4** パフォーマンス確認（Canvas 描画が 16ms/frame 以内）
