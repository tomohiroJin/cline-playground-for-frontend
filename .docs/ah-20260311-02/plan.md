...
**目的**: Phase 1 完了後のコードレビューで検出された品質・安全性の問題を修正

**レビュー実施日**: 2026-03-13

**作業内容**:
1. **[Critical] `story.ts` の `loadStoryProgress` に try-catch を追加**
   - localStorage 破損時のクラッシュ防止
   - 簡易バリデーション付きのフォールバック処理
2. **[High] `useImagePreloader` のクリーンアップ改善**
   - `img.src = ''` によるリソース解放
   - `console.warn` によるエラーログ追加
3. **[High] `DialogueOverlay` の eslint-disable 削除・依存配列修正**
   - useEffect の依存配列を正しく指定
4. **[High] `useImagePreloader` の eslint-disable 削除・依存配列修正**
   - `urls` と `urlsKey` の同期問題を解消
5. **[High] `scoreRef` / `scores` 二重管理への注意喚起コメント追加**
6. **[Medium] 勝利カットイン画像パスの一元化**
   - `AirHockeyGame.tsx` と `get-stage-asset-urls.ts` の重複解消
7. **[Medium] `AirHockeyGame.tsx` リファクタリング方針の記録**
   - `useStoryMode`, `useScreenTransition` への将来的な分離案をドキュメント化
8. **[Low] テストの `data-testid` 使用箇所の見直し**
9. **[Low] `VictoryCutIn` テストの `global.Image` モックリストア確認**

**成果物**: 修正されたソースファイル群 + テスト全パス確認
**完了条件**: 全指摘事項が対応済み、既存テスト + 新規テスト全パス
