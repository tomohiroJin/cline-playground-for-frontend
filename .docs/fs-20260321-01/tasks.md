...
### 7-1. ディレクトリ構造の最終調整
- [ ] 目標アーキテクチャのディレクトリ構造に整合していることを確認
- [ ] 不要になった旧ファイルの削除
  - 旧 `grid.ts`（ルート）
  - 旧 `block.ts`（ルート）
  - 旧 `bullet.ts`（ルート）
...
### 7-2. 公開 API の整理
- [ ] `index.ts` のエクスポートを新しいモジュールパスに更新
- [ ] 旧エクスポート（非推奨）の削除
- [ ] 他の feature からの参照がないことを確認

### 7-3. コード品質チェック
- [ ] ESLint を実行し、エラー・警告がないことを確認
- [ ] TypeScript の型チェック（`tsc --noEmit`）がパスすることを確認
- [ ] 未使用の import/変数がないことを確認

### 7-4. 全テスト実行
- [ ] 全テストを実行し、すべてパスすることを確認
- [ ] カバレッジレポートを最終確認
