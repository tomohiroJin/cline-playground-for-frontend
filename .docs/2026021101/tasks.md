...
- [ ] `highScore` 更新時にゲーム状態が初期化される問題を修正
- [ ] RST/ESC のタイトル遷移時にハイスコア保存判定を追加
- [ ] 仮想ボタン入力に `touchcancel` 解放処理を追加
- [ ] ステージ進行を「固定時間被弾/固定時間クリア」から「ハザード接触/目標達成」へ変更
- [ ] 追加回帰テストを作成（状態継続、保存、進行）
