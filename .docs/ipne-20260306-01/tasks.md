...
- [ ] **LevelBadge の CSS 位置変更**（`IpnePage.styles.ts` 673行目付近）
  - [ ] `top: 0.75rem` → `top: clamp(2.5rem, 5vmin, 3.5rem)` に変更（ボタン行の下に移動）
  - [ ] `right: 4rem` → `right: clamp(0.5rem, 1.5vmin, 1rem)` に変更（右端に寄せて統一）
- [ ] **StatsDisplay の CSS 位置変更**（`IpnePage.styles.ts` 623行目付近）
  - [ ] `top: 3.5rem` → `top: clamp(3.5rem, 7vmin, 5rem)` に変更（LevelBadge の下に移動）
  - [ ] `right: 1rem` → `right: clamp(0.5rem, 1.5vmin, 1rem)` に変更（clamp() でレスポンシブ化）
- [ ] **ボタン群は変更なし**を確認（MapToggleButton / HelpButton / KeyIndicator / PendingPointsBadge）
- [ ] **画面幅別の表示確認チェックリスト**
...
- [ ] **戦士スプライト再デザイン**（方向の明確化）
  - [ ] 下向き（正面）: 顔の特徴（目・バイザー）を明確化、胸当てのV字ライン
  - [ ] 上向き（背面）: マントの質感、背面の盾を明確化
  - [ ] 左向き: 横顔シルエット、武器を手前に描画
  - [ ] 右向き: 武器持ち手を考慮した独自デザイン
  - [ ] 全方向で明らかに異なるシルエットを確保
- [ ] **盗賊スプライト再デザイン**（方向の明確化）
  - [ ] 下向き: 尖ったフード、光る目を明確化
  - [ ] 上向き: クロークの背面を明確化
  - [ ] 左向き・右向き: ダガーの持ち方を明確化
- [ ] パレット12色の有効活用（ハイライト・影・アクセント）

### 0C-3. プレイヤーアニメーションフレーム数強化 [P1]

- [ ] 歩行アニメーション 3フレーム → 4フレーム化
  - [ ] 中間フレーム（idle2）の追加（全方向 × 2キャラ = 8フレーム新規作成）
  - [ ] `frameDuration` を 150ms → 120ms に調整
- [ ] 攻撃アニメーションのフレーム数維持（2フレーム）
- [ ] アイドルブリーズのフレーム数維持（1フレーム）

### 0C-4. 敵スプライト品質向上 + フレーム数強化 [P1]

- [ ] **パトロール（スライム）**: 2F → 4F、品質向上（ぷるぷる質感）
- [ ] **チャージ（ビースト）**: 2F → 4F、品質向上（突進の迫力）
- [ ] **レンジ（メイジ）**: 2F → 4F、品質向上（杖の発光変化）
- [ ] **スペシメン**: 2F → 4F、品質向上（不気味な脈動）
- [ ] **ミニボス**: 2F → 4F、品質向上
- [ ] **メガボス**: 2F → 4F、品質向上
- [ ] `frameDuration` の調整（spec.md 0C.4 参照）

### 0C-5. 環境・エフェクトスプライト品質向上 [P2]

- [ ] タイルスプライトのディテール改善（目地・テクスチャの描き込み）
- [ ] 壁スプライトの品質向上（ひび割れ・レンガの質感）
- [ ] エフェクトスプライトの品質向上（軌跡の滑らかさ）

### 0C-6. テスト・動作確認 [P0]

- [ ] ビルド・起動確認（コンパイルエラーなし）
- [ ] HUD要素の重なりが解消されていること
- [ ] プレイヤーの方向が4方向で明確に判別できること
- [ ] 歩行アニメーションが滑らかであること
- [ ] 敵アニメーションが滑らかであること
...
```
[P0] 0A. レスポンシブCanvas化 + HUDレイアウト再設計 ← 最優先（画面改善の前提）
  ├─ 0A-1. viewport.ts リファクタリング
  ├─ 0A-2. useCanvasSize フック新規作成
  ├─ 0A-3. IpnePage.styles.ts レイアウト変更
  ├─ 0A-4. Game.tsx Canvas動的サイズ対応
  ├─ 0A-5. HUDレイアウト再設計
  └─ 0A-6. テスト・動作確認
  ↓
[P0] 0B. スプライト解像度アップ（16→32）← 表示サイズ確認しながら調整
  ├─ 0B-1. 基盤変更（config.ts、パレット拡張）
  ├─ 0B-2. プレイヤースプライト再設計（56スプライト）
  ├─ 0B-3. 通常敵スプライト再設計
  ├─ 0B-4. ボス系スプライト再設計
  ├─ 0B-5. 環境スプライト再設計
  ├─ 0B-6. アイテム・エフェクトスプライト再設計
  └─ 0B-7. テスト・動作確認
  ↓
[P0] 0C. スプライト品質向上 + アニメーション強化 ← フィードバック対応
  ├─ 0C-1. HUD右上の重なり修正
  ├─ 0C-2. プレイヤースプライト品質向上（方向明確化）
  ├─ 0C-3. プレイヤーアニメーションフレーム数強化（3→4F）
  ├─ 0C-4. 敵スプライト品質向上 + フレーム数強化（2→4F）
  ├─ 0C-5. 環境・エフェクトスプライト品質向上
  └─ 0C-6. テスト・動作確認
  ↓
[P1] 2-1. フローティングダメージ → 基盤（FloatingTextManager）を他機能が参照
  ↓
[P1] 2-1b. 敵攻撃エフェクト可視化 → 敵攻撃が見えない問題の解消
  ↓
[P1] 2-4. 敵撃破演出 → 爽快感の基盤
  ↓
[P2] 2-2. ヒットエフェクトスケーリング → パワーレベル計算を他機能が参照
  ↓
[P2] 1-1. パワーオーラ → キャラクターの見た目改善
  ↓
[P2] 2-3. コンボシステム → 撃破演出との連携
  ↓
[P2] 2-5. ボス戦演出 → ボスBGMがステージBGMに依存
  ↓
[P3] 1-2. 武器エフェクト
  ↓
...
| フェーズ | タスク数 | 重要度 |
|---------|---------|--------|
| Phase 0A（レスポンシブ+HUD） | 6セクション | 最優先（前提） |
| Phase 0B（解像度アップ 16→32） | 7セクション | 最優先（前提） |
| Phase 0C（品質向上+アニメ強化） | 6セクション | 最優先（FB対応） |
| Phase 2（戦闘演出） | 6セクション | 高 |
| Phase 1（ビジュアル） | 3セクション | 高 |
| Phase 3（体験向上） | 4セクション | 中 |
| 横断（テスト・ドキュメント） | 2セクション | 必須 |
| **合計** | **34セクション** | |
