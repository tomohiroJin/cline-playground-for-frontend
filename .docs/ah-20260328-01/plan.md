...
- [x] すべてのフェーズが完了（S5-1〜S5-12: 2026-03-28〜29）
- [x] 既存テスト全パス（85 スイート / 1256 テスト）
- [x] 型エラーなし（`tsc --noEmit`）
- [x] ESLint エラーなし
- [x] ビルド成功（`npm run build`）
- [x] ペアマッチの一連フロー（チーム設定 → VS → ゲーム → リザルト → チーム設定に戻る / リプレイ）が動作
- [x] P2 CPU/人間切り替えが動作
- [x] マレット間衝突判定が動作
- [x] パックスタック脱出が動作
- [x] 2v2 ゴールサイズ拡大が動作
- [x] キーマッピング分離（2P/2v2 で WASD が P1 に影響しない）
- [x] キャラ別 AI プロファイルが反映される
...
- **確認事項**: `resolveMalletMalletOverlaps` のゾーン再クランプが新しい 2 分割ゾーンで正しく機能するか検証
- **確認事項**: `updateExtraMalletAI` の `getPlayerZone('player2')` クランプが 2 分割後も正しいか
...
**ゴールサイズ設計（デザインレビュー精査済み）**:

一律倍率ではなく「マレット 3 つ分（252px）以上」を下限保証する:

```typescript
const PAIR_MATCH_MIN_GOAL_SIZE = MALLET_RADIUS * 2 * 3; // 252px
const pairMatchGoalSize = Math.max(field.goalSize * 1.5, PAIR_MATCH_MIN_GOAL_SIZE);
```

| フィールド | 元サイズ | ×1.5 | 下限保証後 | カバー率（2マレット） |
|-----------|---------|------|----------|-------------------|
| classic | 160 | 240 | **252** | 67% |
| pillars | 160 | 240 | **252** | 67% |
| zigzag | 180 | 270 | 270 | 62% |
| fortress | 140 | 210 | **252** | 67% |
| bastion | 160 | 240 | **252** | 67% |
| wide | 240 | 360 | 360 | 47% |

**修正方針**:
- `startGame` 時にフィールド設定を immutable コピーし `goalSize` のみ上書き
- ゴールのレンダリング・ゴール判定・ゴールエフェクトは `field.goalSize` から参照するため自動的に連動
- Canvas 幅（600px）に対するゴール上限チェック: `Math.min(goalSize, W * 0.6)` で 360px 以下に制限

**影響ファイル**: `presentation/AirHockeyGame.tsx`（フィールドコピー + goalSize 上書き）
...
## Phase S5-12: プレイテストバグ修正（2026-03-29）

### 発見された問題

| # | 問題 | 種別 | 今回対応 |
|---|------|------|---------|
| PB-1 | CPU 同士でパックを挟んで永久ループ | バグ | **修正** |
| PB-2 | ゴールサイズ拡大が機能しない | バグ | **修正** |
| PB-3 | CPU 戦略が同じ動きになる | 改善要望 | **ドキュメント記録（後工程）** |

### PB-1: パック永久ループ対策

**原因**: `processCollisions()` で全マレットを順次チェックする際、マレット A がパックを押し出した先で
マレット B が同フレーム内で再度パックを押し戻す。2 マレットが近接していると毎フレーム繰り返される。
...
**現状**: `buildFreeBattleAiConfig` でキャラ ID に基づく `playStyle` を設定しているが、
`CpuAI.calculateTargetWithBehavior` の実装では `aggressiveness`・`oscillation` 程度の差しかなく、
実際のプレイで目に見える個性の差が出にくい。

**将来の改善候補**:
- キャラごとに異なる**守備位置パターン**（センター守備型 / サイド張り型）
- パック予測の**精度差**を `wobble` だけでなく予測方向の偏りで表現
- 攻撃時の**打ち返し角度のクセ**（ストレート / クロス / ランダム）
- **リアクション速度**（パックが来てから動き出すまでのディレイ）
- team 単位の**連携 AI**（味方がカバーしている時は攻めに出る）

> この改善は AI アーキテクチャの大きな変更を伴うため、
> ペアマッチ基盤の完成後に専用フェーズとして計画する。

---

## ブランチ完了サマリー（2026-03-29）

### 実施フェーズ一覧

| Phase | 内容 | 完了日 |
|-------|------|--------|
| S5-1 | 状態管理の拡張（useGameMode） | 2026-03-28 |
| S5-2 | TeamSetupScreen 機能拡充 | 2026-03-28 |
| S5-3 | VsScreen 2v2 対応 | 2026-03-28 |
| S5-4 | ゲーム開始フロー接続 | 2026-03-28 |
| S5-5 | ResultScreen 2v2 最適化 | 2026-03-28 |
| S5-6 | テスト・品質保証 | 2026-03-28 |
| S5-7 | P2 CPU/人間切り替え | 2026-03-28 |
| S5-8 | デザインレビュー UI/UX 改善 | 2026-03-28 |
| S5-9 | デザイン残課題 レスポンシブ・UX 磨き込み | 2026-03-28 |
| S5-10 | ゲームプレイ品質改善 | 2026-03-28 |
| S5-11 | プレイテストフィードバック対応 | 2026-03-29 |
| S5-12 | プレイテストバグ修正 | 2026-03-29 |
...
本ブランチで検出・記録した将来対応項目を、優先度順に整理する。

### 優先度: 高

| # | 項目 | カテゴリ | 詳細 |
|---|------|---------|------|
| NEXT-1 | VsScreen 2v2 レスポンシブ対応 | UI | 立ち絵サイズが固定（256px×4）でモバイル画面ではみ出す。`min()` / `clamp()` での縮小が必要（DR-1） |
| NEXT-2 | CPU 戦略の深い個性化 | ゲームロジック | 守備位置パターン、打ち返し角度のクセ、リアクション速度、連携 AI（PB-3） |

### 優先度: 中

| # | 項目 | カテゴリ | 詳細 |
|---|------|---------|------|
| NEXT-3 | フォントサイズのスケール統一 | デザインシステム | 8 種類散在（10/11/12/14/16/20/24/72px）→ 5 段階程度に整理 |
| NEXT-4 | キャラ選択パネルの開閉アニメーション | UX | `max-height` 200ms ease-out（DR-2） |
| NEXT-5 | グリッド展開時の自動スクロール | UX | `scrollIntoView`（DR-3） |
| NEXT-6 | ResultScreen 2v2 チーム間区切りの精緻化 | UI | チーム内 gap / チーム間 gap の調整 |
| NEXT-7 | P3/P4 の人間操作対応 | 機能拡張 | ゲームパッド API / 追加キーマッピング導入後に対応（FB-3） |
| NEXT-8 | ペアマッチ中断時の設定保持 | UX | `resetToFree` ではなく確認ダイアログ検討（SR-5） |

### 優先度: 低

| # | 項目 | カテゴリ | 詳細 |
|---|------|---------|------|
| NEXT-9 | VsScreen 1v1 の reduced-motion 完全対応 | アクセシビリティ | `CharacterPanel` 内 transition が 1v1 でも常に有効（対応済みだが個別パネルの確認が必要）（DR-6） |
| NEXT-10 | パフォーマンス最適化 | パフォーマンス | Chrome DevTools / React Profiler でボトルネック計測（FB-4） |
| NEXT-11 | useGameMode の肥大化解消 | リファクタリング | 27 フィールド → サブオブジェクトにグループ化。DDD リファクタリング（Phase R）と連動 |
| NEXT-12 | 色定義の一元管理 | デザインシステム | 312 箇所以上に散在する色値の CSS カスタムプロパティ化 |
| NEXT-13 | 2v2 マレットカラーの差別化 | UI | P1 と P2 が同じ領域で動くため、ally マレットにキャラカラーを反映 |

### 調査記録: 不安定テスト疑い（2026-03-29）

コードレビュー時のテスト実行で `Test Suites: 1 failed` が 1 回発生。
再現試行（単体 10 回 + 全体 10 回 + 推定原因の検証テスト）では一度も再現せず。
推定原因（`Math.random` / `Date.now` の非決定性）を検証テストで否定済み。
並行ファイル編集中の一時的なコンパイルエラーが最も可能性の高い原因。
次回発生時はエラーメッセージの全文を即座に記録して調査すること。
