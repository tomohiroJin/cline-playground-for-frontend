...
- [ ] **S5-8-1**: チーム1/チーム2 セクションの色分け（青/赤の左ボーダー + タイトル色）
  - 対象: `components/TeamSetupScreen.tsx`
  - チーム1: `#3498db`（青系）、チーム2: `#e74c3c`（赤系）
- [ ] **S5-8-2**: P1 固定スロットの改善（`opacity: 1` + `cursor: default` + ラベル強調）
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S5-8-3**: 難易度セクションをチーム構成の上に移動
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S5-8-4**: キャラ選択パネルの開閉アニメーション（`max-height` 200ms ease-out）
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S5-8-5**: キャラ選択グリッド展開時の自動スクロール（`scrollIntoView`）
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S5-8-6**: テスト — TeamSetupScreen の色分け・P1 表示・セクション順序
  - 対象: `components/TeamSetupScreen.test.tsx`

### VsScreen 改善

- [ ] **S5-8-7**: 2v2 時の立ち絵サイズをレスポンシブ化（`min()` で縮小）
  - 対象: `components/VsScreen.tsx`
  - 480px 未満で縦並びレイアウトに切り替え
- [ ] **S5-8-8**: `prefers-reduced-motion` 対応（アニメーションスキップ）
  - 対象: `components/VsScreen.tsx`
- [ ] **S5-8-9**: 2v2 時のチームラベル表示（「チーム1」「チーム2」）
  - 対象: `components/VsScreen.tsx`
- [ ] **S5-8-10**: テスト — VsScreen のレスポンシブ・reduced-motion・ラベル
  - 対象: `components/VsScreen.test.tsx`

### ResultScreen 改善

- [ ] **S5-8-11**: 2v2 時の立ち絵 4 体表示（P1+P2 / P3+P4）
  - 対象: `components/ResultScreen.tsx`
  - `allyCharacter` / `enemyCharacter2` Props を追加
- [ ] **S5-8-12**: `AirHockeyGame` から ResultScreen へ ally/enemy2 キャラを渡す
  - 対象: `presentation/AirHockeyGame.tsx`
- [ ] **S5-8-13**: テスト — ResultScreen の 2v2 立ち絵 4 体表示
  - 対象: `components/ResultScreen.test.tsx`

### 品質保証

- [ ] **S5-8-14**: テスト・品質保証（型チェック・テスト全パス・ビルド成功）
  - 対象: 全体
