...
- 画面遷移フロー（タイトル → ストーリー → ゲーム → 結果）の E2E テストを試みたが、Canvas 描画の初期化待ちが CI 環境で不安定（タイムアウト・リトライで CI が長引く問題が発生）
- 単体テスト（177件）とブラウザ手動確認で品質を担保する方針に変更
...
### Phase 6: 最終整備
- ~~画面遷移の E2E テスト追加~~ → CI 環境で不安定のため削除
- index.ts のエクスポート整理
- 既存テストの最終リファクタリング
