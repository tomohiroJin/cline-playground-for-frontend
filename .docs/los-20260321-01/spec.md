...
**対象**: DOM ベースの画面遷移フローのみ（Canvas 描画は対象外）

```
e2e/labyrinth-of-shadows/
  screen-flow.spec.ts    # 画面遷移テスト
```
...
| テスト | 内容 |
|-------|------|
| タイトル画面表示 | タイトル文字、難易度ボタン3つが表示される |
| 難易度選択 → ストーリー画面 | 難易度ボタンクリック後、ストーリーテキストが表示される |
| ストーリースキップ → ゲーム画面 | スキップボタンクリック後、Canvas が表示される |
| ポーズ機能 | Escape キーでポーズモーダルが表示/非表示される |
| ポーズから再開 | 再開ボタンクリック後、ポーズモーダルが消える |
| ポーズからタイトルへ | タイトルボタンクリック後、タイトル画面に戻る |
