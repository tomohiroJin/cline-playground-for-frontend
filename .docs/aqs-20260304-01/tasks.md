...
- [x] `character-genre-map.ts` を新規作成
  - [x] 5キャラクター×ジャンルのマッピング定義
  - [x] マッピング取得関数（`getGenresForCharacters`）

#### 11.2 StudySelectScreen UI変更
- [x] キャラクターカード選択セクションの追加（5キャラ横並び）
- [x] キャラクター選択時の関連ジャンル自動選択ロジック
- [x] 複数キャラクター選択時の和集合処理
- [x] キャラクター選択解除時のジャンル解除処理
- [x] 手動ジャンル追加・削除との共存ロジック

#### 11.3 テスト
- [x] `character-genre-map.ts` のユニットテスト（21テスト）
  - [x] 全キャラクターのマッピングが定義されていること
  - [x] マッピングのジャンルIDが `TAG_MASTER` に存在すること
- [x] `StudySelectScreen.tsx` のコンポーネントテスト更新（11テスト）
  - [x] キャラクター選択でジャンルが自動選択されること
  - [x] 複数キャラクター選択で和集合になること
  - [x] キャラクター選択後に手動でジャンル変更できること
- [x] `npm test` で全テストがパス（189スイート / 2679テスト）
