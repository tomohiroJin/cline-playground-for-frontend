...
- [ ] **S6-3-17**: [R-3] キャラ特性アイコンのマッピング定義を追加
  - 対象: `core/character-ai-profiles.ts` or `core/config.ts`
  - attacker→⚔️, defender→🛡️, balanced→⚖️ + 対応色
- [ ] **S6-3-18**: [R-3] TeamSetupScreen のキャラスロットに特性バッジ（16x16px）を表示
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S6-3-19**: [R-3] キャラ選択グリッドにも特性バッジを表示
  - 対象: `components/TeamSetupScreen.tsx`
- [ ] **S6-3-20**: テスト — 各キャラに正しい特性アイコンが表示される
  - 対象: テストファイル

### S6-3h: 品質保証

- [ ] **S6-3-21**: テスト・品質保証（型チェック・テスト全パス・ビルド成功）
  - 対象: 全体
