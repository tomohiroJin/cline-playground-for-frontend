...
# 段階A の実施記録（2026-09-04〜09-06）

> **この節は実行後に追記した。** 台帳（`.superpowers/` 配下・git 追跡外）にしか無かった
> 裁定と繰り越しを、コミットされる場所へ移したものである。
> **最終レビューの指摘 I7 の根本原因がこれだった**——裁定で `levy_chosen` を外したのに、
> コミットされた記録（設計書 §9.1）は古いまま残り、段階C の計画がそれを入力にするところだった。

## 結果

- **15タスクすべて完了。** `npm run ci` 緑（809スイート / 9,610テスト）
- **段階A の完了条件7件すべて充足**
- **ただし `G1` は判定不能。** 段階B へは進まない（設計書 §8.2.4）
- 修正ループに入ったタスク: 8件（Task 4・5・8・9・10・11・12・13・15）
- 最終レビューで Critical 1件・Important 8件を検出し、すべて処理済み

## 実行中に下した裁定（全件）

形式: 何を決めたか — なぜ — 誤りだった場合のコスト

- worktree を作らず、feature ブランチ上の作業ツリーで実行する — 実装者は逐次で並行しないため worktree が防ぐ同時変更の risk が無く、別ツリーでは node_modules が無いので全タスクの検証ステップ（npm test / npm run ci）が動かない — 誤りなら、サブエージェントが作業ツリーを汚した場合に本チェックアウトへ波及する（各タスクの git status ゲートで検出する）
- T5 の移行表から `useAshenRampartGame.test.ts:56` の `iteration: 5 → 6` を削除し、Task 12 へ移した — `CURRENT_ITERATION` を上げるのは T12 なので、T5 で期待値だけ先に変えると定数が 5 のまま赤くなる — 誤りなら T12 で二重に直すだけで済む（実害は小さい）
- simulateExpedition を位置引数4つから ExpeditionSimulationInput のオブジェクト引数へ変更（計画の Task 13/14/15 を修正） — .claude/rules/coding-style.md の「パラメータは3個以内、超える場合はオブジェクトにまとめる」に違反しており、かつ strategy と acquire がどちらも関数で取り違えても型が通る — 誤りなら呼び出しがやや冗長になるだけ（実害なし）
- Task 4 と Task 5 の実行順を入れ替える — Task 4 のテストヘルパは maxCopiesOf 依存で、DECK_SIZE=20・魔力炉無制限の状態では上限違反を作れず「同名上限は実行時も守る」が落ちる（事前スキャンで私が DECK_SIZE=12 を前提に検算した誤り） — 誤りなら Task 4 のテストを書き換える手戻りが出るが、依存は一方向（Task 5 は validateRuntimeDeck を参照しない）なので入れ替えは安全
- Task 12 から levy_chosen を外し、Task 15 の再生操作列からも choose-levy を外す — Task 5 で徴発が retired になり validateDeck が弾くため startRunWithDeck 経由ではデッキに入らず、産出者がいないイベント型になる（expedition_* を段階C へ送ったのと同じ理由） — 誤りなら反復7 で徴発を戻すときに一緒に入れれば済む。domain の徴発ロジックと step-tick-levy.test.ts は生きている
- 徴発の UI 経路（LevyChoice.tsx・chooseLevy・levyOptions）が到達不能な死にコードになった件は段階C（UI 作り直し）で扱う — 段階A でこれを消すと反復7 で徴発を戻すときに作り直しになる — 誤りなら段階C まで未使用コードが残るだけ
- プリセット較正を20枚版のローカル凍結（LEGACY_PRESET_DECKS）へ向け直させる — 本番 PRESET_DECKS が12枚になったため describe 全体が「12枚デッキを1160tick 単一ランで回す」というどこにも存在しない構成を測っており、it.failing が救ったのは1件だけで残り3件は通っているが意味を失っている（このプロジェクトが戦ってきた空虚なテストそのもの） — 誤りなら20枚版の凍結コードが余分に残るだけで、12枚版の較正は段階D で別ファイルに作る
- 設計書 §10.4 の assertion（padToDeckSize は入力を切り捨てていない）を、恒真式から「生成器を実際に通して各札の枚数が減っていないことを検査する」形へ訂正 — レビューが指摘したとおり cards.length <= DECK_SIZE は恒真で padToDeckSize を呼びもせず、検出対象の欠陥を原理的に捉えられない。設計書が指定したコードがそのまま計画・実装へ流れた — 誤りなら検査がやや冗長になるだけ
- この plan-mandated な重複は修正する — .claude/rules/coding-style.md が DRY を明示的に求めており、重複を正当化する設計上の理由が無い（私がブリーフを書くとき validateDeck をコピーしただけ）。片方だけ直して静かに乖離する経路が実在する — 誤りなら非公開ヘルパ2本が増えるだけで、振る舞いは変わらない
- clamp を削るのではなくテストを足す — RandomFn の契約は 0..1 未満で本番では到達しないが、同じ形の clamp を buildOffer(Task 9) と randomAcquireOf(Task 13) でも使っており、削ると3箇所を変えたうえでテストスタブが壊れやすくなる — 誤りなら到達しないコードとその検査が残るだけ
- 同じ欠陥を Task 9/13 の計画へ先回りで反映（clamp テストを追加） — Task 8 で1件見つかった以上、同型が2箇所にあることが分かっているのに実装後に同じ指摘を2回受けるのは無駄 — 誤りなら不要なテストが2本増えるだけ
- ガードを足す — このモジュールは以降の全タスク（use-case・シミュレーション・G1・完全再生）が乗る土台で、CLAUDE.md が DbC を設計原則に掲げている。3遷移のうち2つに部分的なガードがあって1つに無いのは非対称 — 誤りなら例外が1つ増えるだけで、正当な呼び出しは影響を受けない（Task 11 の advanceStage は既に phase を確認してから presentOffer を呼ぶ設計）
- 実装者が見つけたブリーフの実行時バグ2件の修正を受け入れる — (1) startStage のガードが敗北終了を検出できなかった（敗北では stageIndex が進まないため currentStage が負けたステージを返し続ける。私のブリーフの見落とし）、(2) advance-stage.test が chooseAcquisition を手で模す際に acquired の更新を忘れ、offerIndex が両回とも 0 になっていた（同上） — どちらも修正が正しく理由も明記されている。誤りならレビューが指摘する
- Important 1 に加えて Minor 4件（ACQUIRED_INSERT_OFFSET の未検査／複数獲得と clamp の未検査／docstring の tick 数値のずれ／waves の弱い assertion）も同ラウンドで修正させる — 未検査の定数は Task 8・9 と同型で3度目、かつ G1 の要となるファイル。docstring の数値の誤りはこのプロジェクトが最も重く扱う「記録に事実でないことを書く」型 — 誤りなら修正が数行増えるだけ
- 私が M1 で例示した assertion（indexOf > length/2）は誤りだった — OFFSET=3 なら添字7、OFFSET=0 なら添字10 で、どちらも length/2 を超えるため変異を殺せない。実装者が実測で見抜き設計値を直接埋める形へ差し替えたのを受け入れる — 誤りならレビューが指摘する
- applyAcquisition の提示メンバーシップ検査を外しても Task 11 のテストが落ちない件は追加対応しない — その検査は acquisition.test.ts が守っており層として正しい配置。Task 11 のテストは常に正当な提示メンバーを渡すので落ちないのが想定どおり。Important 1 の狙いは手組みのドリフト解消であり、それは達成された — 誤りなら E2E の穴が残るが、domain 側で検査済み
- 同じ実装者を再開せず、新しい実装者へ引き継ぐ — 2度の再開で収束せず、コンテキストが待機ループで汚れている。作業ツリーの未コミット変更は破棄しない（実装は妥当に見えるので、引き継いだ実装者が検証してコミットする） — 誤りなら新しい実装者が問題を見つけて報告する
- 元の実装者の成果を採用し、引き継ぎエージェントは破棄する — 元の実装者がコミット・レポート・npm run ci まで完了させており、引き継ぎ側が変異テストで作業ツリーを触ると競合する — 誤りなら二重作業のコストだけ（成果物は失われない）
- handIndex を FIFO キューではなくドメインの discarded イベントに載せる（domain 層の改修を許可） — レビュアーが通常操作での再現を示した（同一 tick 内に添字0と添字2の捨札 → 1件目で配列が縮み2件目が範囲外で黙って拒否 → キューに余った値が以降すべてに付きラン全体でずれる）。既存の discarded イベントの docstring 自身が「成立を知っているのはドメインだけなので、ここでイベントとして名乗る」と述べており、添字も同じ理由でドメインが名乗るべき。FIFO キューはその discarded が解決したはずの問題を添字について再導入している — 誤りなら domain 層の変更コストだけ（step-tick-actions.test.ts の更新を含む）
- 「domain 層は非改修」という私の指示が不明確だったことを認め、このタスクでの domain 変更を許可する — タスクの目的（ログから再生できること）を満たすには domain の変更が必要 — 誤りなら domain の TickEvent が1フィールド増えるだけ
- 実装者に npm run ci を実行させず、コントローラが後でまとめて実行する — Task 12・13 の2回とも同じ待機ループで停止した。対象ファイル指定の npx jest なら30秒以内で変異確認には十分 — 誤りなら lint/build の問題の発見が最終レビュー直前まで遅れる（そこで検出できる）
- 条件付き assertion を修正させる — このファイルは Task 14 の G1 ゲートの測定器であり、静かに無効化される経路（段階B・D の較正）はこの反復がこれから通る道そのもの。同型の欠陥は5タスク連続 — 誤りなら else 分岐が2つ増えるだけ
- 上記 Important は修正しない — 指摘対象は gitignore されたワークスペース内レポートの散文で、成果物はクリーン。判断は私が独立に下している — 誤りなら実装者への境界の周知が1回弱まるだけ
- G1 の判断を「通過」から「判定不能」へ撤回する — 事前登録した §8.2 第2条（差を勝率ではなく反実仮想で測る。同一シード・同一操作列を再生し獲得札だけを抜く）を測定が実行していなかった。しかも「合格条件は満たされた」と書くときその条文を引いていない（反復5 §1.6 の選択的引用と同型の再発）。加えて n=40 の検出力が 8.1%、中心的主張（獲得は期待値を上げない）が n=600 で覆る、randomAcquire の値が12本中2本の当たりストリーム、§7.1 の自己違反、§7.10 の必須欄が空 — 誤りなら段階B の開始が遅れるが、誤った因果を土台に段階B/D を積むより安い
- 段階B へ進まない。Task 15 を先に完成させ、その完全再生の機構で G1 を反実仮想で測り直す — §8.2 第2条が要求しているのがまさに Task 15 の機構である — 誤りなら Task 15 の実装が G1 に流用できないと判明するだけ
- G1 のやり直しでは、判定の演算・最小効果量・標本サイズ・多重比較の扱いを測る前に別コミットで事前登録する — 今回は「差が出るか」の定義が無いまま観測後に読み方を作れる状態だった — 誤りなら事前登録の手間が1回増えるだけ
- offerIndex を completed.stageIndex - 1（これまでに提示した回数）へ変更させる — これは私が Task 11 のブリーフに「獲得の回数であって stageIndex ではない」と書いた指示の誤り。G1 のやり直し（反実仮想）でも提示が腕ごとに違えば測定が成立しない — 誤りなら提示の系列が変わるだけで、獲得の機構自体には影響しない
- 上記2件は修正ラウンドにせず、設計書 §8.2.4(g) に記録して G1 やり直しの入力にする — レビュアー自身が「作ったものの欠陥ではなく、何を証明できたかのギャップ」と判定しており、Task 15 の成果物は正しい — 誤りならやり直しの設計時に気づく

## 繰り越した Minor（全件）

**最終レビューが triage 済み。** 「段階B の前に直す」とされた6件は処理済みで、以下は繰り越し確定分である。

- **Task 1**: レーンの中間座標を検査していない（長さと終端のみ）。順序入れ替えは捕まらない。計画の Step 1 コード由来
- **Task 1**: docstring の「33ファイル/326箇所/26箇所」はレビューで再検証していない（範囲外）
- **Task 2**: index/seed に NaN/Infinity/非整数のランタイムガードが無い（掃引では実害なしを確認済み）
- **Task 2**: 目的間の独立性の検査が shuffle vs offer の1組のみ。stage-draw が他と比較されていない
- **Task 3**: BUILDABLE/ACQUIRABLE の2テストは現時点で同語反復（計画が宣言済みの空虚さの範囲内）。Task 5 Step 8 の変異でこの2つが実際に反転することを確認すること
- **Task 5**: OFF_PATH_DOMINANT_DECK の docstring が「これはルール上まったく合法である」と現在形のまま（「反復5 時点では」を足す）
- **Task 5**: DeckBuilder.test.tsx:105/144 のテスト名が「20枚」のまま（assertion は DECK_SIZE 参照に修正済み）
- **Task 5**: deck-builder.ts の acquire-only 分岐は現在どのカードも通らずテストも無い（段階B で acquire-only の札が入るとき同時に足す）
- **Task 5**: AshenRampartGame.test.tsx の新テストは13枚になるため、枚数違反だけでも成立する（1枚減らしてから徴発を足せば2つの assertion が独立に効く）
- **Task 5**: AshenRampartGame.test.tsx の buildCustomDeck が fireEvent.click 12行ベタ書き。startRunningWith*Deck の2名が同一関数の別名で誤読を招く
- **Task 5**: useAshenRampartGame.test.ts の seed:59 が3箇所にリテラル散在（名前付き定数にすると理由を1箇所に書ける）
- **Task 5**: validateDeck が本体33行で30行目安超過（countByCard のループに入手経路チェックを相乗りできる）
- **Task 5**: 本番コメントの20枚残存 4箇所（DeckBuilder.tsx:4 / card-text.ts:5 / AshenRampartGame.tsx:347 / run-simulation.ts:345）。特に run-simulation.ts:345 は較正の根拠を述べている
- **Task 4**: RUNTIME_DECK_MAX = DECK_SIZE + 2 の 2 が裸のリテラル（ACQUISITIONS_PER_EXPEDITION 等の名前付き定数にできる）
- **Task 4**: 「RUNTIME_DECK_MAX は DECK_SIZE + 2」テストは定数の定義の再主張で同語反復。変異を捕まえない
- **Task 4**: base() の自己検査が5つの n 値のうち2つしか見ていない（手計算では全5値正しいことをレビュアーが確認済み）
- **Task 6**: STAGE_CLEAR_HEAL の「正の整数」テストは定数の再主張で振る舞いを検査しない（符号は設計の一部なので Minor 止まり）
- **Task 7**: stage-pool.test.ts の「軸の本数が層番号と一致」「同じ層の2ステージは組み合わせが異なる」は authored data の検査でロジックを守らない（段階B の差し替え時のデータ入力ミスは捕まえる）
- **Task 7**: BLOCK_HP_THRESHOLD=40 は 17〜59 の広い空隙にあり、特定カードに固定されていない（heavy-hit の 12 は火砲台に固定）。将来 hp 17〜39 のカードが来たとき恣意的になる
- **Task 9**: 「retired は提示しない」テストの識別力は間接的（責務は card-pool.ts 側なので実装としては正しい）
- **Task 9**: clamp が下限（負値）を見ていない（RandomFn の契約外）
- **Task 10**: phase と outcome が独立フィールドで、不正な組み合わせが型として表現可能（判別共用体にすれば構築不能にできるがブリーフの指定外）
- **Task 10**: 踏破時に stageIndex が stages.length（範囲外）に残り currentStage が undefined を返す。未テスト
- **Task 11**: M2 の複数獲得テストは i 項を丸ごと外す変異（position を pile.length - OFFSET に固定）を検出できない。Math.max(0,...) の clamp も未検査
- **Task 11**: 「無効な札を選ぶと拒否される」契約は Task 11 のテスト単体ではカバーされず domain 層テストのみが守る（層としては正しいが記録）
- **Task 12**: local-storage-play-log.test.ts の新 describe が既存テストと assertion を一部重複
- **Task 12**: presentation 側の再現テストは単一イベントのため、FIFO キューを再導入しても落ちない（余った値が壊すのは「後続の無関係な捨札」であり、そのテストは後続を実行しない）。domain 側のテストは落ちるので実害は無いが記録
- **Task 13**: simulateExpedition の本体が38行で目安30行を超過（オファー解決部を抽出できる）
- **Task 13**: '獲得の提示中ではありません' が presentOffer / declineOffer / chooseAcquisition の3箇所に重複（4つ目が来たら定数化）
- **Task 14**: レポートの散文が §8.2 の判断表のどの行に該当するかを名指しした（境界の踏み越え。コード・設計書には及ばず）

## 段階B の入力として記録しておくこと

- **`BLOCK_HP_THRESHOLD = 40` は 17〜59 の広い空隙にある**（`heavy-hit` の 12 は火砲台に固定されている）。
  段階B で HP 17〜39 の守り手を作った瞬間に、その `block` 適格性が恣意的になる
- **`card-pool.test.ts` の「コスト4以上を1枚以上」は swift で境界ちょうど**（`piercer` 1枚のみ）。
  これ以上緩められない。プリセットを組み直すときに再検討する
- **暫定ステージ6つのうち4つで、宣言した要求軸が実際には要求されていない**
  （`offPathOnly` が層1・層2 の4ステージで 15/15 完勝）。段階B の本番ステージで解消すること
- **徴発の UI 経路が死にコード**（`LevyChoice.tsx` / `chooseLevy` / `levyOptions` / 
  `run-summary.ts` の `pending` / `rejection-text.ts`）。段階C で一括処理する
- **`run` / `stage` / `expedition` の語彙が揃っていない**（`simulateRun` / `runId` / `run_tally` が
  指すのは遠征導入後は「1ステージ」）。設計書 §9.1 は段階C で `expeditionId`＋`stageIndex` へ置き換えると宣言済み
- **`expedition-gate.test.ts` が CI に31秒を払う**（`expedition-replay.test.ts` は18秒）。
  設計書 §8.3 の較正予算は60秒。判定不能となった測定を常駐させるかを `G1` やり直しの設計で決める
