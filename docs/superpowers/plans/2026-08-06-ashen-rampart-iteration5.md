...
**Architecture:** 敵に `attackRange` を持たせ、経路外の守り手にもダメージが届くようにする（変更1）。溢れ1枚につきライフを1点引き、ライフを「序盤の余剰を終盤の飢餓へ運ぶ交換レート」にする（変更2）。両プリセットにコスト4以上を供給する（変更3）。ドメインは純粋関数のまま保ち、`balance.test.ts` の不変条件6本で較正を守る。
...
| `domain/combat/balance.test.ts` | 較正の不変条件 | 不変条件を6本に（1本追加）、閾値を再測定 |
...
## Task 9: 較正 — `balance.test.ts` の不変条件6本
...
- 較正の不変条件を6本に（新規: 配備後に何もしない戦略が勝てない）
