...
`run-simulation.ts`: **単に `placeableCells(...)[0]` を取ってはいけない。**

設置マス22の規則を廃したことで、`(0,0)` のような**どの守り手の射程も経路に届かないマス**が候補に入るようになった。素直に先頭を取ると、シミュレーションは「盤面の隅に塔を並べた勝率」を測ることになり、Task 14 の較正が丸ごと無意味になる。

**役割に応じた置き場所を選ぶ。** 賢い戦略を作るのではなく、「雑に遊んでも最低限まともな場所には置く」下限を保つための処置である。
...
const choosePlacement = (
  ...
  // 射程（オーラ分は見ない。素の射程で届く場所を選ぶ）が経路に届くマスのうち、最も近いも
...
const restrictedGreedy = (
  ...
  for (let handIndex = 0; handIndex < state.deck.hand.length; handIndex++) {
    ...
    // Task 8 の choosePlacement を述語で絞った版。役割に応じた置き場所を選ぶ
