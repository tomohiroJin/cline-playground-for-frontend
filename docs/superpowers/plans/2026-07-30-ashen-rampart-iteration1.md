...
- [ ] **Step 5: プリセット2種を14種前提で再構成 → Task 9 へ移動（実行中に判明した順序ミス）**

> **この Step は Task 9 で実施する。** プリセットに新カードを入れると、振る舞いが未実装（Task 4〜6）の段階で `balance.test.ts` が全ラン敗北で赤になり、**Task 3〜8 のあいだ赤いテストを抱えたまま進む**ことになる。その間に別の回帰が入っても検出できない。
>
> Task 3 ではカード定義の追加のみを行い、**プリセットは既存カードのままにする**。再構成は振る舞いが揃った Task 9 で較正と同時に行う。
>
> 以下の構成は Task 9 で使う。各20枚・同名3枚以内。**プリセット間の差が魔力炉の数だけにならないよう、答えの選び方を変える。**
...
- [ ] **Step 0: プリセット2種を14種前提で再構成（Task 3 から移動）**

Task 3 の Step 5 に記載した構成をここで適用する。新カードの振る舞いが揃ってから行うことで、`balance.test.ts` が赤になる期間を作らない。

**2つのプリセットで対空の答えが違う**（速攻型＝弩砲 / 重厚型＝落網＋徹甲弩）ことが要点。PoC ではどちらも弩砲を積むしかなかった。

再構成後は既存のプリセットテスト（20枚ちょうど・同名3枚以内・構築規則を満たす）が通ることを確認する。
