...
- 成功基準（`'burst'` 戦略・トーテム平均勝率）:
  - 無強化 di0 ∈ [0.40, 0.65]（原始でも苦戦）。
  - 無強化 di1 < 0.30（ツリーなしは厳しい）。
  - フル強化 di3 ∈ [0.55, 0.80]（必勝でない）。
  - 断崖なし: 無強化で隣接難易度の勝率低下が緩やか（di0→di1→di2 の各差 ≤ 0.40）。
  - (b) パワーカーブ終盤交差（bc0/bc1 blood>ember、bc2 ember>blood）維持。
- 狂気倍率: 狂血(madblood) ×2→×1.6、儀式(rit)低HP ×3→×2（合算上限 ×6→×3.2）。最終値は較正で微調整可。
- `any` 型禁止。コメントは日本語で「なぜ」。色相/表示文字列の整合を保つ。
- バランスは経験的反復で収束。数値は出発点、ガードレール緑が受け入れ基準。
