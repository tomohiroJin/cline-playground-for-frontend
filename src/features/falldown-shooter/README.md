...
- **連鎖崩壊（Chain Collapse）**: 撃破・着地・スキルでセルが消えると列重力が働き、
  「同色の連結グループ（4個以上）」または「揃った完全行」が消えて上のセルが落下。
  その結果さらに条件が揃えば連続で消える**多段連鎖**が発生する。連鎖数に応じて
  スコア倍率（最大8倍）・効果音のピッチ・画面演出が段階的に増幅する。
...
- **連鎖解決**: 列重力＋同色グループ／完全行の判定を行う純粋関数 `resolveBoard`
