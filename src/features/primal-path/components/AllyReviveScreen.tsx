...
export const AllyReviveScreen: React.FC<Props> = ({ run, dispatch, playSfx, showOverlay }) => {
  ...
  return (
    <Screen $center>
      ...
      <Divider />
      <div style={{ fontSize: IFS.md, color: '#908870', marginBottom: 6 }}>所持骨: <Gc>{run.bE}</Gc></div>
      ...
      {dead.map((a, i) => {
        ...
        return (
          <GamePanel key={i} style={{ padding: 10, textAlign: 'center', margin: '4px 0' }}>
            <div style={{ fontSize: IFS.lg, color: TC[a.t], marginBottom: 4 }}>💀 {a.n}</div>
            <div style={{ fontSize: IFS.sm, color: '#605848', marginBottom: 6 }}>
              HP {a.mhp} / ATK {a.atk}{a.tk ? ' (盾役)' : a.h ? ' (回復役)' : ''}
            </div>
            <div style={{ display: 'flex', gap: 6, justifyContent: 'center' }}>
              <GameButton $off={!canHalf} style={{ fontSize: IFS.md, minWidth: 100 }}
                onClick={() => canHalf && handleRevive(i, 50)}>
                HP50%復活<br />
                <span style={{ fontSize: IFS.sm, color: canHalf ? '#f0c040' : '#f05050' }}>🦴{cost50}</span>
              </GameButton>
              <GameButton $off={!canFull} style={{ fontSize: IFS.md, minWidth: 100 }}
                onClick={() => canFull && handleRevive(i, 100)}>
                HP100%復活<br />
                <span style={{ fontSize: IFS.sm, color: canFull ? '#f0c040' : '#f05050' }}>🦴{cost100}</span>
              </GameButton>
            </div>
          </GamePanel>
        );
