...
export const TreeScreen: React.FC<Props> = ({ save, dispatch, playSfx, showOverlay }) => {
  ...
  return (
    <Screen>
      ...
      <div style={{ fontSize: IFS.sm, color: '#605848', marginBottom: 2 }}>
        取得 {boughtN}/{totalN} ・クリア{save.clears}回
      </div>
      {boughtN > 0 && hasBonus && (
        <div style={{ fontSize: IFS.md, color: '#aaa', marginBottom: 4, textAlign: 'center' }}>
      ...
      {[1, 2, 3, 4, 5, 6, 7, 8].map(tier => {
        ...
        return (
          <React.Fragment key={tier}>
            ...
            {!unlocked ? (
              <div style={{ fontSize: IFS.sm, color: '#401820', textAlign: 'center', marginBottom: 4 }}>{req}回クリアで解放</div>
            ) : (
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 3, justifyContent: 'center' }}>
                {nodes.map(nd => {
                  ...
                  return (
                    ...
                    >
                      <div style={{ color: bought ? '#50a050' : cc, fontSize: IFS.sm }}>{nd.n}</div>
                      <div style={{ fontSize: IFS.xs, color: '#605848', margin: '1px 0' }}>{nd.d}</div>
                      {req2 && !bought && <div style={{ fontSize: IFS.xs, color: '#444' }}>要:{req2.n}</div>}
                      <div style={{ fontSize: IFS.sm, color: bought ? '#3a6a3a' : canBuy ? '#f0c040' : '#444' }}>
                        {bought ? '✓' : `🦴${nd.c}`}
                      </div>
                    </TreeNodeBox>
                  );
