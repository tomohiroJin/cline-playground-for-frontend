...
import type { SfxType, CivType } from '../types';
import type { GameAction } from '../hooks';
import { EVOS, SYNERGY_BONUSES, SYNERGY_TAG_INFO } from '../constants';
import { IFS } from '../constants/ui';
import { Screen, SubTitle, Divider, GameButton, GamePanel, TabBtn } from '../styles';

/** 文明タイプの表示情報 */
const CIV_LABELS: Record<CivType, { label: string; color: string }> = {
  tech: { label: '技術', color: '#f08050' },
  life: { label: '生活', color: '#50e090' },
  rit: { label: '儀式', color: '#d060ff' },
};

/** フィルタ選択肢 */
const FILTER_OPTIONS: readonly (CivType | 'all')[] = ['all', 'tech', 'life', 'rit'] as const;
const FILTER_LABELS: Record<string, string> = {
  all: '全て', tech: '技術', life: '生活', rit: '儀式',
};
...
const RulesTab: React.FC = () => (
  <GamePanel style={{ fontSize: IFS.md, lineHeight: 1.9, padding: '10px 12px' }}>
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🌍 基本ルール</p>
    <p>3バイオーム踏破→最終ボス。進化選択→自動戦闘。</p>
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>⚡ 三大文明</p>
    ...
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>✦ バイオーム相性</p>
    <p>最も高い文明がバイオームと合致すると<span style={{ color: '#50e090' }}>ATK×1.2〜1.3</span>のボーナス！進化・バイオーム選択画面で確認できます。</p>
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🔥 覚醒システム</p>
    ...
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>✦ アクティブスキル</p>
    ...
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>⏩ 戦闘速度</p>
    <p>速度ボタン（×1/×2/×4/×8）で自動戦闘のスピードを変更。</p>
    <p>⏸ボタンで一時停止。速度設定はセッション中のラン間で保持されます。</p>
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🧬 シナジーシステム</p>
    ...
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🗺️ ランダムイベント</p>
    <p>バトル後に一定確率でランダムイベントが発生します。</p>
    <p>骨の商人、古代の祠、毒沼など全8種。選択肢によって異なる効果が得られます。</p>
    <p>バイオームごとに出現しやすいイベントがあります。</p>
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🏆 実績・チャレンジ</p>
    ...
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>⚔ 戦闘のコツ</p>
    <p>降伏すると骨が半減。復活ツリーで死亡時に復活可能。</p>
    <p>仲間は文明Lv2/4/6で加入。盾役はダメージを吸収。</p>
    <p>倒れた仲間はバイオーム踏破時に骨を使って復活可能。</p>
    <p>レア進化「魂呼びの儀」「再誕の祈り」でも蘇生。</p>
    <br />
    <p style={{ color: '#f0c040', fontSize: IFS.lg, marginBottom: 2 }}>🦴 骨と文明ツリー</p>
    <p>死亡/クリアで骨獲得。永続強化。<br />難易度クリアで上位ティアが解放！</p>
  </GamePanel>
);

/** 進化図鑑タブ */
const EvoDexTab: React.FC = () => {
  const [filter, setFilter] = useState<CivType | 'all'>('all');
  const filtered = filter === 'all' ? EVOS : EVOS.filter(ev => ev.t === filter);
    ...
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4, justifyContent: 'center' }}>
        {filtered.map((ev, i) => {
          ...
          return (
            <GamePanel key={i} style={{ width: '45%', padding: '6px 8px', fontSize: IFS.sm }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 2 }}>
                <span style={{ color: civInfo.color, fontWeight: 'bold', fontSize: IFS.md }}>{ev.n}</span>
                <span style={{ fontSize: IFS.xs, color: civInfo.color, border: `1px solid ${civInfo.color}40`, padding: '0 3px', borderRadius: 2 }}>
                  {civInfo.label}
                </span>
              </div>
              <div style={{ color: '#808068', marginBottom: 2 }}>{ev.d}</div>
              {ev.tags && ev.tags.length > 0 && (
                <div style={{ display: 'flex', gap: 2, flexWrap: 'wrap' }}>
                  {ev.tags.map(tag => {
                    const info = SYNERGY_TAG_INFO[tag];
                    return info ? (
                      <span key={tag} style={{ fontSize: IFS.xs, color: info.cl }}>
                        {info.ic}{info.nm}
                      </span>
                    ) : null;
                  })}
...
const SynergyTab: React.FC = () => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
    {SYNERGY_BONUSES.map(sb => {
      ...
      return (
        <GamePanel key={sb.tag} style={{ padding: '6px 8px', fontSize: IFS.sm }}>
          <div style={{ color: info?.cl || '#f0c040', fontSize: IFS.lg, marginBottom: 3 }}>
            {info?.ic} {info?.nm}
          </div>
          <div style={{ marginBottom: 2 }}>
            <span style={{ color: '#50e090', fontSize: IFS.sm }}>Tier1（2個）</span>{' '}
            <span style={{ color: '#f0c040' }}>{sb.tier1.name}</span>
            <div style={{ color: '#808068', marginLeft: 8 }}>{sb.tier1.description}</div>
          </div>
          <div>
            <span style={{ color: '#f0c040', fontSize: IFS.sm }}>Tier2（3個+）</span>{' '}
...
/** タブ定義 */
const TABS = ['基本ルール', '進化図鑑', 'シナジー'] as const;
...
export const HowToPlayScreen: React.FC<Props> = ({ dispatch, playSfx }) => {
  ...
  return (
    <Screen>
      ...
      {/* タブ切替 */}
      <div style={{ display: 'flex', gap: 4, justifyContent: 'center', margin: '4px 0 6px' }}>
        {TABS.map((label, i) => (
          <TabBtn key={label} $active={activeTab === i} onClick={() => handleTabChange(i as 0 | 1 | 2)}>
            {label}
          </TabBtn>
        ))}
      </div>
      ...
      {activeTab === 1 && <EvoDexTab />}
      {activeTab === 2 && <SynergyTab />}
