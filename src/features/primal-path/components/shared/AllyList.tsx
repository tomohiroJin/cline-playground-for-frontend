          ...
          return (
            <AllyBadge key={i} $dead={!a.a}>
              <canvas
                ...
                style={{ width: 36, height: 48, margin: '0 auto 2px', display: 'block', imageRendering: 'pixelated' }}
              />
              <div style={{ color: TC[a.t] }}>{a.n}</div>
              {a.a && (
                <div style={{ height: 5, background: '#1a1a22', borderRadius: 2, overflow: 'hidden', marginTop: 1, width: '100%' }}>
                  <div style={{ height: '100%', borderRadius: 2, transition: 'width .2s', width: `${hpPct}%`, background: hpCl }} />
                </div>
              )}
              <div style={{ fontSize: IFS.sm, color: '#605848' }}>{a.a ? `${a.hp}/${a.mhp}` : '💀'}</div>
            </AllyBadge>
          );
        }
        return (
          <AllyBadge key={i} $dead={!a.a}>
            ...
            <span style={{ fontSize: IFS.sm, color: '#605848' }}>{a.a ? `HP${a.hp}` : '💀'}</span>
          </AllyBadge>
        );
      })}
    </AllyRow>
