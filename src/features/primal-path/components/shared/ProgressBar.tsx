...
import { IFS, ELLIPSIS_STYLE } from '../../constants/ui';
...
export const ProgressBar: React.FC<ProgressBarProps> = ({ current, max, label }) => {
  ...
  return (
    <div style={{ width: '100%', marginBottom: 5 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: IFS.sm, color: '#605848', marginBottom: 1 }}>
        <span style={{ maxWidth: '45%', ...ELLIPSIS_STYLE }}>{label}</span><span>{current}/{max}</span>
      </div>
