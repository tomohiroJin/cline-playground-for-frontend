...
describe('classifyTeamType - チームタイプ分類', () => {
  ...
  it('シナジーチームの境界値: stab=65, debt=20, tp=60', () => {
    const stats: ClassifyStats = { stab: 65, debt: 20, emSuc: 0, sc: [60], tp: 60, spd: 7 };
    expect(classifyTeamType(stats).id).toBe('synergy');
  });

  it('シナジーチームの境界外: stab=64', () => {
    const stats: ClassifyStats = { stab: 64, debt: 20, emSuc: 0, sc: [60], tp: 60, spd: 7 };
    expect(classifyTeamType(stats).id).not.toBe('synergy');
  });

  it('シナジーチームの境界外: debt=21', () => {
    const stats: ClassifyStats = { stab: 70, debt: 21, emSuc: 0, sc: [60], tp: 60, spd: 7 };
    expect(classifyTeamType(stats).id).not.toBe('synergy');
  });

  it('レジリエントチームの境界値: emSuc=2', () => {
    const stats: ClassifyStats = { stab: 30, debt: 30, emSuc: 2, sc: [40], tp: 40, spd: 8 };
    expect(classifyTeamType(stats).id).toBe('resilient');
  });

  it('レジリエントチームの境界外: emSuc=1', () => {
    const stats: ClassifyStats = { stab: 30, debt: 30, emSuc: 1, sc: [40], tp: 40, spd: 8 };
    expect(classifyTeamType(stats).id).not.toBe('resilient');
  });

  it('アジャイルチームの境界値: spd=5.5, tp=50', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [55, 55], tp: 50, spd: 5.5 };
    expect(classifyTeamType(stats).id).toBe('agile');
  });

  it('アジャイルチームの境界外: spd=5.6', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [55, 55], tp: 50, spd: 5.6 };
    expect(classifyTeamType(stats).id).not.toBe('agile');
  });

  it('もがくチームの境界値: debt=35', () => {
    const stats: ClassifyStats = { stab: 10, debt: 35, emSuc: 0, sc: [30, 30], tp: 30, spd: 10 };
    expect(classifyTeamType(stats).id).toBe('struggling');
  });

  it('もがくチームの境界外: debt=34', () => {
    const stats: ClassifyStats = { stab: 10, debt: 34, emSuc: 0, sc: [30, 30], tp: 30, spd: 10 };
  ...
  it('シナジーがレジリエントより優先される', () => {
    const stats: ClassifyStats = { stab: 70, debt: 15, emSuc: 3, sc: [70, 70], tp: 70, spd: 7 };
    expect(classifyTeamType(stats).id).toBe('synergy');
  });

  it('レジリエントが成長するチームより優先される', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 2, sc: [40, 70], tp: 55, spd: 8 };
    expect(classifyTeamType(stats).id).toBe('resilient');
  });

  it('成長するチームにはsc.length>=2が必要', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [40], tp: 55, spd: 8 };
  ...
  it('結成したてのチームは常に最後のフォールバック', () => {
    ...
    expect(defaultType.condition({ stab: 0, debt: 0, emSuc: 0, sc: [], tp: 0, spd: 0 })).toBe(true);
  });
});
...
describe('classifyEngineerType - エンジニアタイプ分類', () => {
  ...
  it('安定運用型: stab>=65, debt<=20, tp>=60', () => {
    const stats: ClassifyStats = { stab: 70, debt: 15, emSuc: 0, sc: [60, 70], tp: 65, spd: 7 };
  ...
  it('火消し職人: emSuc>=2', () => {
    const stats: ClassifyStats = { stab: 30, debt: 40, emSuc: 2, sc: [40, 50], tp: 45, spd: 8 };
    expect(classifyEngineerType(stats).id).toBe('firefighter');
  });

  it('成長曲線型: sc[0]<50, sc[last]>=65', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [40, 55, 70], tp: 55, spd: 8 };
    expect(classifyEngineerType(stats).id).toBe('growth');
  });

  it('高速レスポンス型: spd<=5.5, tp>=50', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [60, 60], tp: 60, spd: 4.0 };
    expect(classifyEngineerType(stats).id).toBe('speed');
  });

  it('技術的負債と共に生きる人: debt>=35', () => {
    const stats: ClassifyStats = { stab: 10, debt: 40, emSuc: 0, sc: [30, 30], tp: 30, spd: 10 };
    expect(classifyEngineerType(stats).id).toBe('debt');
  });

  it('デフォルト: どの条件にも合わない場合', () => {
    const stats: ClassifyStats = { stab: 50, debt: 25, emSuc: 0, sc: [55, 55], tp: 55, spd: 7 };
