...
const plainsLane = laneOf(PLAINS_MAP, 0);
...
describe('stackEnemies', () => {
  it('生きている敵だけを対象にする', () => {
    ...
    expect(stackEnemies([dead], plainsLane)).toEqual([]);
  });
  ...
  it('種別が違えば束ねない', () => {
    const stacks = stackEnemies([enemy(1, 'swarm', 1.0), enemy(2, 'grunt', 1.0)], plainsLane);
    expect(stacks).toHaveLength(2);
  });

  it('離れていれば束ねない', () => {
    const stacks = stackEnemies([enemy(1, 'swarm', 1.0), enemy(2, 'swarm', 5.0)], plainsLane);
    expect(stacks).toHaveLength(2);
  });

  it('HPはスタック内の合計になる', () => {
    const stacks = stackEnemies([enemy(1, 'swarm', 1.0, 8), enemy(2, 'swarm', 1.1, 8)], plainsLane);
