...
describe('nextWavePreview', () => {
  it('tick 0 では最初の次ウェーブ（雑兵3 俊足2）が予告される', () => {
    expect(nextWavePreview({ waves: PLAINS_WAVES, tick: 0 })).toBe('雑兵3 俊足2');
  });

  it('tick が進んでも次ウェーブ開始 tick を超えなければ予告は変わらない', () => {
    expect(nextWavePreview({ waves: PLAINS_WAVES, tick: 249 })).toBe('雑兵3 俊足2');
  });
