...
const withUnits = (units: { cardId: string; x: number; y: number }[]): CombatState => ({
  ...
  units: units.map((u) => ({ cardId: u.cardId, pos: { x: u.x, y: u.y }, hp: 10, maxHp: 10, cooldownLeft: 0 })),
});

describe('effectiveRange', () => {
  it('支援が無ければカード定義の射程そのまま', () => {
    const state = withUnits([{ cardId: 'arrow-tower', x: 1, y: 2 }]);
  ...
  it('オーラの守り手自身の実効射程は 0', () => {
    const state = withUnits([{ cardId: 'forge', x: 1, y: 2 }]);
