...
/**
 * 較正ハーネスそのものの性質（**設計の不変条件ではない**）
 *
 * ここに置く2本は「ゲームがこうあるべき」という主張ではなく、**測定器
 * （`deployThenIdleStrategy` と `DEPLOY_ONLY_UNTIL_TICK`）がどういう性質を
 * 持っているかの記録**である。下の `較正の不変条件` の describe とは意味が違うので
 * 混ぜないこと。ここが赤くなっても「ゲームが壊れた」ではなく「測定器の前提が
 * 変わった」を意味する（例: 山札の枚数やドロー間隔を変えた）。
 *
 * **反復5 の計画（設計書 §10.1・§10.3）は、この1本目を「配備後に何もしない戦略は
 * 4/20 未満」という6本目の不変条件へ反転させるつもりだった。それは撤回された。**
 * 反転できないのは数値の問題ではなく、この戦略が測っているものが「配備後の判断」
 * ではないからである（実測はすべて `deployThenIdleStrategy` の docstring にある）。
 *
 * - `DEPLOY_ONLY_UNTIL_TICK`(680) は「山札が尽きる tick」として定義されている
 * - その時点で20シードすべて山札は0枚・手札は0〜2枚
 * - 素直な戦略が 680 以降に打つのは20ラン合計12枚（1ランあたり0.6枚）
 * - **20シード中11シードでは素直な戦略も 680 以降に一切操作せず、2つの戦略の
 *   ランは完全に一致する**（その11シードの勝ち数も greedy 7・idle 7 で同じ）
 *
 * つまりこの戦略が放棄しているのは平均0.6枚の札であって、配備後の判断ではない。
 * 41通りのウェーブ・射程の組み合わせを掃引しても、全要求充足デッキが 12/20 以上に
 * 留まる限りこの戦略は 8/20 を下回らなかった（詳細は task-9-report.md）。
 *
 * **「配備が終わった後にも判断が残るか」は自動検証から実プレイの判定へ移した**
 * （設計書 §8.2 の判定項目5 `unitsLost` と 6「最後に出した tick ÷ 決着 tick」）。
 * 札を抱える人間のプレイでは、この戦略が測れなかったものが実際に測れる。
 */
describe('較正ハーネスの性質（測定器の記録であって設計の要求ではない）', () => {
  it('配備後に何もしない戦略は、素直な戦略とほぼ同じ結果になる（放棄しているのは平均0.6枚だけ）', () => {
    const idle = winsOf(FULL_DECK, deployThenIdleStrategy, 'deployThenIdle');
    const greedy = winsOf(FULL_DECK);
    // 実測 12/20 対 10/20（差2）。**この差が小さいことは設計の欠陥ではない。**
    // tick 680 で山札が尽きる以上、2つの戦略はほぼ同じ入力列になる（上の docstring）。
    // ここで固定しているのは「両者がほぼ同じ結果になる」という測定器の性質であり、
    // 差が広がったら（＝供給を終盤へ伸ばす変更が入ったら）気づけるように上限を置く。
    // greedy の掃引は runAllSeeds のキャッシュに載るので実行コストは増えない
 ...
 *
 * **⚠️ 反復5 の判定後の敵対的検証で、上表の「経路外のみ 0/20」は
 * FULL_DECK でしか成り立たないことが判明した。** 塔中心の合法デッキ
 * （`OFF_PATH_DOMINANT_DECK`）では **greedy 20/20・offPathOnly 12/20** で、
 * 不変条件を3倍超で破る。**1デッキで測った値に全称の主張を載せていた。**
 * 反復6 で解消する（判定記録 2026-08-14 §4.3・§5.4）。
 *
 * 反復5 の再測定は、摩耗（敵の射程攻撃）・溢れのライフ対価・速攻型への徹甲弩
 * 追加がすべて入った後の値。動かしたのは waves.ts の数とタイミングだけで、
 * 閾値は1つも動かしていない。
 *
 * **本数は5本のまま。** 計画にあった6本目（配備後に何もしない戦略が負ける）は
 * 撤回した。あの戦略は「配備後の判断」を測っていなかったためで、理由は上の
 * `較正ハーネスの性質` の docstring にある。代わりに実プレイの判定項目5・6 が
 * 同じことを測る（設計書 §8.2）。
 */
describe('較正の不変条件（反復3・反復5 で再測定）', () => {
  it('全要求充足デッキは 12/20 以上・18/20 以下で勝つ', () => {
    ...
    //
    // **警告: 反復5 の実測は 12/20 で、下限ちょうど・余裕ゼロである。**
    // 敵・カード・ウェーブを難しい側へ動かす変更を入れると、その瞬間にここが割れる。
    // 余裕を作れなかったのは較正の手抜きではなく、13〜14/20 に上げた組み合わせが
    // すべて「プリセット2種の差5以内」か「経路外のみ 4/20 未満」のどちらかを
    // 割ったため（41通りの掃引記録は task-9-report.md）。難度を上げる変更を
    // するときは、ここを同時に見直す前提で臨むこと。
  ...
  it('全要求充足デッキでは、経路上に一切置かない戦略は 4/20 未満しか勝てない', () => {
    // デッキ構成では検査できない不変条件。すべての守り手がブロックできる以上、
    // 石壁を抜いたデッキでも弓兵を経路上に置けば成立してしまう。行為のほうを絞る。
    //
    // **⚠️ 反復5 の敵対的検証で、この条件は FULL_DECK でしか成り立たないことが
    // 判明した。** タイトルは「経路上に一切置かない戦略は…」という全称の主張
    // だったが、検査していたのは FULL_DECK 1本だけである。塔中心のデッキでは
    // 12/20 勝つ（下の2件）。**1デッキで測った値に全称のタイトルを付けない。**
    expect(winsOf(FULL_DECK, offPathOnlyStrategy, 'offPath')).toBeLessThan(4);
  });

  /**
   * 本来立てたい不変条件。**いまは破れているので `failing` で印を付ける。**
   *
  ...
  it.failing('どのデッキでも、経路上に一切置かない戦略は 4/20 未満しか勝てない', () => {
    expect(
      winsOf(OFF_PATH_DOMINANT_DECK, offPathOnlyStrategy, 'offPathDominant')
    ).toBeLessThan(4);
  });
...
describe('較正の基準値', () => {
  it('敵の総HPが 808 から変わっていない', () => {
    ...
    // 敵数を変えたら描画密度（スタック表示）を再計算すること
    // （PoC 設計書 2026-07-29-ashen-rampart-realtime-deck-poc-design.md §9.3。
    // 異種の敵が重なる場合はスタック集約では救えない。waves.ts の docstring 参照）
