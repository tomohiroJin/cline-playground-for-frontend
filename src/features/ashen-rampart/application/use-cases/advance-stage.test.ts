...
describe('advanceStage', () => {
  it('勝つと3択が提示される', () => {
    const after = advanceStage(startExpedition(preset, 7, randomFactory), { won: true, lifeLeft: 9 }, randomFactory);
    expect(after.phase).toBe('offer');
    expect(after.offer).toHaveLength(OFFER_SIZE);
  });

  it('負けると提示は無く遠征が終わる', () => {
    const after = advanceStage(startExpedition(preset, 7, randomFactory), { won: false, lifeLeft: 0 }, randomFactory);
    expect(after.phase).toBe('ended');
    expect(after.offer).toEqual([]);
  });

  it('最終ステージに勝つと踏破で、提示は無い', () => {
    let exp = startExpedition(preset, 7, randomFactory);
    ...
    exp = advanceStage(exp, { won: true, lifeLeft: 7 }, randomFactory);
    exp = chooseAcquisition(exp, exp.offer[0]!);
    exp = advanceStage(exp, { won: true, lifeLeft: 5 }, randomFactory);
    expect(exp.outcome).toBe('cleared');
    expect(exp.offer).toEqual([]);
  });

  it('同じ遠征・同じ回の提示は毎回同じ（決定的）', () => {
    const a = advanceStage(startExpedition(preset, 7, randomFactory), { won: true, lifeLeft: 9 }, randomFactory);
    const b = advanceStage(startExpedition(preset, 7, randomFactory), { won: true, lifeLeft: 9 }, randomFactory);
    expect(a.offer).toEqual(b.offer);
  });

  it('1回目と2回目の提示は独立（同じ添字の派生シードを使い回していない）', () => {
    let exp = advanceStage(startExpedition(preset, 7, randomFactory), { won: true, lifeLeft: 9 }, randomFactory);
    const first = exp.offer;
    // chooseAcquisition を通すと acquired が伸び、次の offerIndex が進む。
    exp = chooseAcquisition(exp, first[0]!);
    const second = advanceStage(exp, { won: true, lifeLeft: 7 }, randomFactory).offer;
