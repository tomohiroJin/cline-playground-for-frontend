...
import type { DrawingAPI, SpriteData } from '../types';

/**
 * 描画ヘルパーモジュールを生成する
 * @param $ CanvasRenderingContext2D
 */
export function createRendering($: CanvasRenderingContext2D): DrawingAPI {
  /** LCD on/off カラー */
  const lcdFg = (on: boolean): string => on ? ON : GH;
  const lcdBg = (on: boolean): string => on ? BG : 'rgba(176,188,152,0.3)';
  ...
  /** ストローク円 */
  function circleS(x: number, y: number, r: number): void {
    $.beginPath(); $.arc(x, y, r, 0, TAU); $.stroke();
  }

  /** ON色フィル（アルファ指定） */
  function onFill(alpha: number): void { $.fillStyle = ON; $.globalAlpha = alpha; }

  /** ON色ストローク（アルファ・線幅指定） */
  function onStroke(alpha: number, lw: number = 1): void { $.strokeStyle = ON; $.lineWidth = lw; $.globalAlpha = alpha; }

  /** LCD矩形描画 */
  function R(a: number, b: number, w: number, h: number, on: boolean): void { $.fillStyle = lcdFg(on); $.fillRect(Math.round(a), Math.round(b), w, h); }

  /** テキスト描画 */
  function txt(s: string, a: number, b: number, sz?: number, on?: boolean, al?: CanvasTextAlign): void {
  ...
  /** 中央揃えテキスト */
  function txtC(s: string, a: number, b: number, sz?: number, on?: boolean): void { txt(s, a, b, sz, on, 'center'); }

  /** スプライトピクセル描画。flip=trueで左右反転 */
  function px(data: SpriteData, dx: number, dy: number, s: number, on: boolean, flip?: boolean): void {
  ...
  function drawK(spr: SpriteData, x: number, y: number, s: number, on: boolean, dir: number): void { px(spr, x, y, s, on, dir < 0); }

  /* ============ アイコン描画関数 ============ */
  function iHeart(a: number, b: number, on: boolean): void {
  ...
  function iGem(a: number, b: number, on: boolean): void {
  ...
  function iSlime(a: number, b: number, on: boolean): void {
  ...
  function iGoblin(a: number, b: number, on: boolean): void {
  ...
  function iSkel(a: number, b: number, on: boolean): void {
  ...
  function iBoss(a: number, b: number, on: boolean): void {
